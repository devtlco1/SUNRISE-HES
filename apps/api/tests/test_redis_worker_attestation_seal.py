...
def test_redis_worker_completion_attestation_maps_into_stable_worker_attestation_seal_records_correctly(
    client,
    db_session: Session,
) -> None:
    ...
    completion_attestation = map_worker_response_certification_to_completion_attestation(
        map_worker_response_assurance_to_response_certification(
            map_worker_recovery_verification_to_response_assurance(
                map_worker_remediation_to_recovery_verification(
    ...
    seal = map_worker_completion_attestation_to_attestation_seal(
        completion_attestation
    )

    assert seal.records[0].seal_type == "noop_attestation_seal"
    assert seal.records[1].seal_type == "attestation_seal_summary"
    assert seal.records[2].seal_type == "seal_action_record"
    assert seal.records[3].seal_type == "seal_note"
    assert seal.records[4].seal_type == "final_proof_artifact"
    assert seal.total_records == 5
    assert seal.terminal_records == 4
    assert seal.seal_ready is True


def test_attestation_seal_action_note_and_final_proof_artifacts_are_exposed_correctly(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
    ...
    seal_values = [record.seal_type for record in seal.records]
    assert seal_values == [
        "noop_attestation_seal",
        "attestation_seal_summary",
        "seal_action_record",
        "seal_note",
        "final_proof_artifact",
    ]


def test_lineage_and_correlation_are_preserved_through_worker_attestation_seal_normalization(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
    ...
    assert seal.metadata["source_identifiers"]["job_run_id"] == job_run_id
    assert seal.metadata["source_identifiers"]["command_id"] is not None
    assert seal.metadata["correlation_lineage"]["derived_correlation_id"] == dispatch_request.derived_correlation_id
...
def test_repeated_enqueue_calls_stay_idempotent_with_worker_attestation_seal_generation(
    client,
    db_session: Session,
    monkeypatch,
) -> None:
...
def test_summaries_remain_consistent_after_worker_attestation_seal_generation(
    client,
    db_session: Session,
    monkeypatch,
) -> None:
    ...
    seal = job_run.result_summary["queue_enqueue"]["enqueue_metadata"][
        "redis_worker_attestation_seal"
    ]
    assert seal["records"][0]["seal_type"] == "noop_attestation_seal"
    assert seal["records"][1]["seal_type"] == "attestation_seal_summary"
    assert seal["records"][3]["seal_type"] == "seal_note"
