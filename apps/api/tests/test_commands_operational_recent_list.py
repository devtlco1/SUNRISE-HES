...
def test_recent_commands_read_model_returns_on_demand_read_items(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
    ...
    response = _get_recent_commands(client, token, family="on_demand_read")
...
def test_recent_commands_read_model_returns_mixed_family_projection_including_on_demand_read(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
    ...
    on_demand_template_id = _create_command_template_for_category(
        client,
        token,
        code="commands-recent-mixed-on-demand-read",
        category="on_demand_read",
    )
    on_demand_execute = _execute_on_demand_read_now(
        ...
        endpoint_assignment_id=endpoint_assignment_id,
        protocol_association_profile_id=protocol_profile_id,
        idempotency_key="commands-recent-mixed-on-demand-read-1",
    )
...
def test_recent_commands_read_model_returns_bounded_on_demand_read_response_when_downstream_artifacts_absent(
    client,
    db_session: Session,
) -> None:
    command_id, meter_id, _, _ = _create_submitted_on_demand_read_command(
        client,
        db_session,
        command_template_code="commands-recent-submitted-on-demand-read",
        idempotency_key="commands-recent-submitted-on-demand-read-1",
    )
    token = _login_as_super_admin(client, db_session)
...
def test_recent_commands_read_model_includes_on_demand_read_commands(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
