...
def _get_recent_meter_commands(
    client,
    token: str,
    meter_id: str,
    *,
    ...
    return client.get(
        f"/api/v1/meters/{meter_id}/commands/recent",
        headers={"Authorization": f"Bearer {token}"},
        params=params,
    )


def test_meter_scoped_recent_commands_returns_profile_capture_items(
    client,
    db_session: Session,
) -> None:
    ...
    template_id = _create_command_template_for_category(
        client,
        token,
        code="meter-commands-recent-profile-capture",
    ...
    execute_now = _execute_profile_capture_now(
        ...
        idempotency_key="meter-commands-recent-profile-capture-1",
    )
    assert execute_now.status_code == 200

    response = _get_recent_meter_commands(client, token, meter_id, family="profile_capture")
...
def test_meter_scoped_recent_commands_returns_relay_control_items(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
    ...
    template_id = _create_command_template_for_category(
        client,
        token,
        code="meter-commands-recent-relay-control",
    ...
    execute_now = _execute_relay_control_now(
        ...
        endpoint_assignment_id=endpoint_assignment_id,
        protocol_association_profile_id=protocol_profile_id,
        idempotency_key="meter-commands-recent-relay-control-1",
    )
    assert execute_now.status_code == 200

    response = _get_recent_meter_commands(client, token, meter_id, family="relay_control")
...
def test_meter_scoped_recent_commands_returns_on_demand_read_items(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
    ...
    template_id = _create_command_template_for_category(
        client,
        token,
        code="meter-commands-recent-on-demand-read",
    ...
    execute_now = _execute_on_demand_read_now(
        ...
        endpoint_assignment_id=endpoint_assignment_id,
        protocol_association_profile_id=protocol_profile_id,
        idempotency_key="meter-commands-recent-on-demand-read-1",
    )
    assert execute_now.status_code == 200

    response = _get_recent_meter_commands(client, token, meter_id, family="on_demand_read")

    assert response.status_code == 200
    payload = response.json()
    assert payload["meter_id"] == meter_id
    assert payload["family_filter"] == "on_demand_read"
    item = next(
        item
        for item in payload["items"]
        if item["command_id"] == execute_now.json()["result"]["command_id"]
    )
    assert item["command_family"] == "on_demand_read"
    assert item["command_category"] == "on_demand_read"
    assert item["command_status"] == "succeeded"
    assert (
        item["runtime_execution_record_id"]
    ...
    assert (
        item["family_specific_outcome_summary"]["on_demand_read_operation"]
        == "read_billing_snapshot"
    )
    assert item["family_specific_outcome_summary"]["snapshot_type"] == "billing"
    assert (
        item["family_specific_outcome_summary"]["on_demand_read_execution_outcome"]
        == "succeeded"
    )


def test_meter_scoped_recent_commands_returns_mixed_family_projection_for_one_meter_only(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
    meter_id = _create_meter_record(client, token)
    other_meter_id = _create_meter_record(client, token)
    endpoint_assignment_id, protocol_profile_id = _attach_runtime_connectivity(db_session, meter_id)
    other_endpoint_assignment_id, other_protocol_profile_id = _attach_runtime_connectivity(
        db_session,
        other_meter_id,
    )
    channel_id = _create_load_profile_channel(client, token, meter_id)
    profile_template_id = _create_command_template_for_category(
        client,
        token,
        code="meter-commands-recent-mixed-profile-capture",
        category="profile_capture",
    )
    relay_template_id = _create_command_template_for_category(
        client,
        token,
        code="meter-commands-recent-mixed-relay-control",
        category="remote_disconnect",
    )
    other_relay_template_id = _create_command_template_for_category(
        client,
        token,
        code="meter-commands-recent-other-meter-relay-control",
        category="remote_disconnect",
    )
    profile_execute = _execute_profile_capture_now(
        ...
        protocol_association_profile_id=protocol_profile_id,
        channel_ids=[channel_id],
        idempotency_key="meter-commands-recent-mixed-profile-capture-1",
    )
    relay_execute = _execute_relay_control_now(
        ...
        endpoint_assignment_id=endpoint_assignment_id,
        protocol_association_profile_id=protocol_profile_id,
        idempotency_key="meter-commands-recent-mixed-relay-control-1",
    )
    other_relay_execute = _execute_relay_control_now(
        client,
        token,
        other_meter_id,
        command_template_id=other_relay_template_id,
        relay_operation="disconnect",
        endpoint_assignment_id=other_endpoint_assignment_id,
        protocol_association_profile_id=other_protocol_profile_id,
        idempotency_key="meter-commands-recent-other-meter-relay-control-1",
    )
    assert profile_execute.status_code == 200
    assert relay_execute.status_code == 200
    assert other_relay_execute.status_code == 200

    response = _get_recent_meter_commands(client, token, meter_id, limit=10)
    ...
    assert other_relay_execute.json()["result"]["command_id"] not in ids
    families = {item["command_family"] for item in payload["items"]}
    assert "profile_capture" in families
    assert "relay_control" in families


def test_meter_scoped_recent_commands_returns_mixed_family_projection_including_on_demand_read(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
    ...
    on_demand_template_id = _create_command_template_for_category(
        client,
        token,
        code="meter-commands-recent-mixed-on-demand-read",
        category="on_demand_read",
    )
    on_demand_execute = _execute_on_demand_read_now(
        client,
        token,
        meter_id,
        ...
        endpoint_assignment_id=endpoint_assignment_id,
        protocol_association_profile_id=protocol_profile_id,
        idempotency_key="meter-commands-recent-mixed-on-demand-read-1",
    )
    assert on_demand_execute.status_code == 200

    response = _get_recent_meter_commands(client, token, meter_id, limit=20)

    assert response.status_code == 200
    payload = response.json()
    ids = {item["command_id"] for item in payload["items"]}
    assert on_demand_execute.json()["result"]["command_id"] in ids
    families = {item["command_family"] for item in payload["items"]}
    assert "on_demand_read" in families


def test_meter_scoped_recent_commands_returns_bounded_response_when_downstream_artifacts_absent(
    client,
    db_session: Session,
) -> None:
    command_id, meter_id, _, _, _ = _create_submitted_profile_capture_command(
        client,
        db_session,
        command_template_code="meter-commands-recent-submitted-profile-capture",
        idempotency_key="meter-commands-recent-submitted-profile-capture-1",
    )
    token = _login_as_super_admin(client, db_session)
...
def test_meter_scoped_recent_commands_returns_bounded_on_demand_read_response_when_downstream_artifacts_absent(
    client,
    db_session: Session,
) -> None:
    command_id, meter_id, _, _ = _create_submitted_on_demand_read_command(
        client,
        db_session,
        command_template_code="meter-commands-recent-submitted-on-demand-read",
        idempotency_key="meter-commands-recent-submitted-on-demand-read-1",
    )
    token = _login_as_super_admin(client, db_session)
    ...
    assert item["family_specific_outcome_summary"]["snapshot_type"] == "billing"
    assert item["family_specific_outcome_summary"]["on_demand_read_execution_outcome"] is None
    assert item["orchestration_artifact_present"] is False
    assert item["terminalization_artifact_present"] is False
    assert item["execute_now_artifact_present"] is False


def test_meter_scoped_recent_commands_includes_on_demand_read_commands(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
    ...
    assert on_demand_response.json()["id"] in ids


def test_meter_scoped_recent_commands_returns_not_found_for_unknown_meter(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)

    response = _get_recent_meter_commands(client, token, str(uuid.uuid4()))
