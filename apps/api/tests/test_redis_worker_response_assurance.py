...
def test_redis_worker_recovery_verification_maps_into_stable_worker_response_assurance_records_correctly(
    client,
    db_session: Session,
) -> None:
    ...
    recovery_verification = map_worker_remediation_to_recovery_verification(
    ...
    assurance = map_worker_recovery_verification_to_response_assurance(
        recovery_verification
    )

    assert assurance.records[0].assurance_type == "noop_response_assurance"
    assert assurance.records[1].assurance_type == "response_assurance_summary"
    assert assurance.records[2].assurance_type == "assurance_action_record"
    assert assurance.records[3].assurance_type == "assurance_note"
    assert assurance.records[4].assurance_type == "post_verification_artifact"
    assert assurance.total_records == 5
    assert assurance.terminal_records == 4
    assert assurance.assurance_ready is True


def test_response_assurance_action_note_and_post_verification_artifacts_are_exposed_correctly(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
    ...
    assurance_values = [record.assurance_type for record in assurance.records]
    assert assurance_values == [
        "noop_response_assurance",
        "response_assurance_summary",
        "assurance_action_record",
        "assurance_note",
        "post_verification_artifact",
    ]


def test_lineage_and_correlation_are_preserved_through_worker_response_assurance_normalization(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
    ...
    assert assurance.metadata["source_identifiers"]["job_run_id"] == job_run_id
    assert assurance.metadata["source_identifiers"]["command_id"] is not None
    assert (
        assurance.metadata["correlation_lineage"]["derived_correlation_id"]
...
def test_repeated_enqueue_calls_stay_idempotent_with_worker_response_assurance_generation(
    client,
    db_session: Session,
    monkeypatch,
) -> None:
...
def test_summaries_remain_consistent_after_worker_response_assurance_generation(
    client,
    db_session: Session,
    monkeypatch,
) -> None:
    ...
    assurance = job_run.result_summary["queue_enqueue"]["enqueue_metadata"][
