...
def _build_attestation_closure_for_retry_job(client, db_session: Session):
    ...
    attestation_closure = map_worker_settlement_attestation_to_attestation_closure(
        map_worker_register_settlement_to_settlement_attestation(
            map_worker_register_completion_to_register_settlement(
                map_worker_register_publication_to_register_completion(
                    map_worker_register_finalization_to_register_publication(
    ...
    return job_run_id, dispatch_request, attestation_closure


def test_settlement_attestation_maps_into_stable_attestation_closure_records(
    client, db_session: Session
) -> None:
    _, _, attestation_closure = _build_attestation_closure_for_retry_job(client, db_session)

    assert attestation_closure.records[0].closure_type == "noop_attestation_closure"
    assert attestation_closure.records[1].closure_type == "attestation_closure_summary"
    assert attestation_closure.records[2].closure_type == "closure_action_record"
    assert attestation_closure.records[3].closure_type == "attestation_closure_note"
    assert attestation_closure.records[4].closure_type == "post_attestation_closure_artifact"
    assert attestation_closure.total_records == 5
    assert attestation_closure.terminal_records == 4
    assert attestation_closure.closure_ready is True


def test_attestation_closure_artifacts_are_exposed_correctly(client, db_session: Session) -> None:
    _, _, attestation_closure = _build_attestation_closure_for_retry_job(client, db_session)

    closure_values = [record.closure_type for record in attestation_closure.records]
    assert closure_values == [
        "noop_attestation_closure",
        "attestation_closure_summary",
        "closure_action_record",
        "attestation_closure_note",
        "post_attestation_closure_artifact",
    ]


def test_lineage_and_correlation_are_preserved_through_worker_attestation_closure(
    client, db_session: Session
) -> None:
    (
        job_run_id,
        dispatch_request,
        attestation_closure,
    ) = _build_attestation_closure_for_retry_job(client, db_session)

    assert attestation_closure.metadata["source_identifiers"]["job_run_id"] == job_run_id
    assert attestation_closure.metadata["source_identifiers"]["command_id"] is not None
    assert (
        attestation_closure.metadata["correlation_lineage"]["derived_correlation_id"]
...
def test_repeated_enqueue_calls_stay_idempotent_with_worker_attestation_closure_generation(
...
def test_summaries_remain_consistent_after_worker_attestation_closure_generation(
    client, db_session: Session, monkeypatch
) -> None:
    monkeypatch.setattr(settings, "queue_backend", "redis_placeholder")
    token = _login_as_super_admin(client, db_session)
    ...
    attestation_closure = job_run.result_summary["queue_enqueue"]["enqueue_metadata"][
