...
def test_redis_worker_response_assurance_maps_into_stable_worker_response_certification_records_correctly(
    client,
    db_session: Session,
) -> None:
    ...
    response_assurance = map_worker_recovery_verification_to_response_assurance(
        map_worker_remediation_to_recovery_verification(
    ...
    certification = map_worker_response_assurance_to_response_certification(
        response_assurance
    )

    assert certification.records[0].certification_type == "noop_response_certification"
    assert certification.records[1].certification_type == "response_certification_summary"
    assert certification.records[2].certification_type == "certification_action_record"
    assert certification.records[3].certification_type == "certification_note"
    assert certification.records[4].certification_type == "completion_certification_artifact"
    assert certification.total_records == 5
    assert certification.terminal_records == 4
    assert certification.certification_ready is True


def test_response_certification_action_note_and_completion_artifacts_are_exposed_correctly(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
    ...
    certification_values = [record.certification_type for record in certification.records]
    assert certification_values == [
        "noop_response_certification",
        "response_certification_summary",
        "certification_action_record",
        "certification_note",
        "completion_certification_artifact",
    ]


def test_lineage_and_correlation_are_preserved_through_worker_response_certification_normalization(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
    ...
    assert certification.metadata["source_identifiers"]["job_run_id"] == job_run_id
    assert certification.metadata["source_identifiers"]["command_id"] is not None
    assert (
        certification.metadata["correlation_lineage"]["derived_correlation_id"]
...
def test_repeated_enqueue_calls_stay_idempotent_with_worker_response_certification_generation(
    client,
    db_session: Session,
    monkeypatch,
) -> None:
...
def test_summaries_remain_consistent_after_worker_response_certification_generation(
    client,
    db_session: Session,
    monkeypatch,
) -> None:
    ...
    certification = job_run.result_summary["queue_enqueue"]["enqueue_metadata"][
