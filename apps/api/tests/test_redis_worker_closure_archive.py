...
def test_redis_worker_ledger_closure_maps_into_stable_worker_closure_archive_records_correctly(
    client,
    db_session: Session,
) -> None:
    ...
    ledger_closure = map_worker_final_attestation_ledger_to_ledger_closure(
        map_worker_attestation_seal_to_final_attestation_ledger(
            map_worker_completion_attestation_to_attestation_seal(
                map_worker_response_certification_to_completion_attestation(
                    map_worker_response_assurance_to_response_certification(
    ...
    archive = map_worker_ledger_closure_to_closure_archive(ledger_closure)

    assert archive.records[0].archive_type == "noop_closure_archive"
    assert archive.records[1].archive_type == "closure_archive_summary"
    assert archive.records[2].archive_type == "archive_register_record"
    assert archive.records[3].archive_type == "retention_note"
    assert archive.records[4].archive_type == "final_retention_artifact"
    assert archive.total_records == 5
    assert archive.terminal_records == 4
    assert archive.archive_ready is True


def test_closure_archive_register_note_and_retention_artifacts_are_exposed_correctly(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
    ...
    dispatch_request = get_dispatch_request_projection(db_session, job_run_id=job_run_id)
    payload = build_queue_enqueue_payload(dispatch_request)
    archive = map_worker_ledger_closure_to_closure_archive(
        map_worker_final_attestation_ledger_to_ledger_closure(
            map_worker_attestation_seal_to_final_attestation_ledger(
                map_worker_completion_attestation_to_attestation_seal(
                    map_worker_response_certification_to_completion_attestation(
    ...
    archive_values = [record.archive_type for record in archive.records]
    assert archive_values == [
        "noop_closure_archive",
        "closure_archive_summary",
        "archive_register_record",
        "retention_note",
        "final_retention_artifact",
    ]


def test_lineage_and_correlation_are_preserved_through_worker_closure_archive_normalization(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
    ...
    assert archive.metadata["source_identifiers"]["job_run_id"] == job_run_id
    assert archive.metadata["source_identifiers"]["command_id"] is not None
    assert archive.metadata["correlation_lineage"]["derived_correlation_id"] == dispatch_request.derived_correlation_id
...
def test_repeated_enqueue_calls_stay_idempotent_with_worker_closure_archive_generation(
    client,
    db_session: Session,
    monkeypatch,
) -> None:
...
def test_summaries_remain_consistent_after_worker_closure_archive_generation(
    client,
    db_session: Session,
    monkeypatch,
) -> None:
    ...
    archive = job_run.result_summary["queue_enqueue"]["enqueue_metadata"][
        "redis_worker_closure_archive"
    ]
    assert archive["records"][0]["archive_type"] == "noop_closure_archive"
    assert archive["records"][1]["archive_type"] == "closure_archive_summary"
    assert archive["records"][3]["archive_type"] == "retention_note"
