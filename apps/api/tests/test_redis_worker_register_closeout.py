...
def test_register_reconciliation_maps_into_stable_register_closeout_records(
    ...
    register_reconciliation = map_worker_retention_register_to_register_reconciliation(
        map_worker_closure_archive_to_retention_register(
            map_worker_ledger_closure_to_closure_archive(
                map_worker_final_attestation_ledger_to_ledger_closure(
                    map_worker_attestation_seal_to_final_attestation_ledger(
    ...
    closeout = map_worker_register_reconciliation_to_register_closeout(register_reconciliation)

    assert closeout.records[0].closeout_type == "noop_register_closeout"
    assert closeout.records[1].closeout_type == "register_closeout_summary"
    assert closeout.records[2].closeout_type == "closeout_action_record"
    assert closeout.records[3].closeout_type == "register_closeout_note"
    assert closeout.records[4].closeout_type == "final_register_summary_artifact"
    assert closeout.total_records == 5
    assert closeout.terminal_records == 4
    assert closeout.closeout_ready is True


def test_register_closeout_action_note_and_final_summary_artifacts_are_exposed_correctly(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
    ...
    dispatch_request = get_dispatch_request_projection(db_session, job_run_id=job_run_id)
    payload = build_queue_enqueue_payload(dispatch_request)
    closeout = map_worker_register_reconciliation_to_register_closeout(
        map_worker_retention_register_to_register_reconciliation(
            map_worker_closure_archive_to_retention_register(
                map_worker_ledger_closure_to_closure_archive(
                    map_worker_final_attestation_ledger_to_ledger_closure(
    ...
    closeout_values = [record.closeout_type for record in closeout.records]
    assert closeout_values == [
        "noop_register_closeout",
        "register_closeout_summary",
        "closeout_action_record",
        "register_closeout_note",
        "final_register_summary_artifact",
    ]


def test_lineage_and_correlation_are_preserved_through_worker_register_closeout_normalization(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
    ...
    assert closeout.metadata["source_identifiers"]["job_run_id"] == job_run_id
    assert closeout.metadata["source_identifiers"]["command_id"] is not None
    assert (
        closeout.metadata["correlation_lineage"]["derived_correlation_id"]
...
def test_repeated_enqueue_calls_stay_idempotent_with_worker_register_closeout_generation(
    client,
    db_session: Session,
    monkeypatch,
) -> None:
...
def test_summaries_remain_consistent_after_worker_register_closeout_generation(
    client,
    db_session: Session,
    monkeypatch,
) -> None:
    ...
    closeout = job_run.result_summary["queue_enqueue"]["enqueue_metadata"][
        "redis_worker_register_closeout"
    ]
    assert closeout["records"][0]["closeout_type"] == "noop_register_closeout"
    assert closeout["records"][1]["closeout_type"] == "register_closeout_summary"
    assert closeout["records"][3]["closeout_type"] == "register_closeout_note"
