...
def bridge_runtime_protocol_execution_observation_to_interpretation(
    session: Session,
    *,
    ...
    if attempt.job_run_id is None:
        raise HTTPException(
            ...
            detail="Runtime protocol interpretation requires an attempt linked to a job run.",
        )
    ...
    existing = _load_runtime_protocol_interpretation(attempt.execution_metadata)
    if existing is not None:
        if existing.session_identifier != payload.session_identifier:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Runtime protocol interpretation is already recorded for another session.",
            )
        if existing.executor_identifier != payload.executor_identifier:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Runtime protocol interpretation is already owned by another executor.",
            )
        return RuntimeProtocolInterpretationBridgeResponse(
    ...
    if session_result is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol interpretation requires a finalized runtime session.",
        )
    if session_result.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol interpretation does not match the finalized runtime session.",
        )
    ...
    if outcome is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol interpretation requires a recorded runtime execution outcome.",
        )
    if outcome.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol interpretation does not match the recorded runtime execution outcome.",
        )
    ...
    if disposition is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol interpretation requires a recorded runtime attempt disposition.",
        )
    if disposition.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol interpretation does not match the recorded runtime attempt disposition.",
        )
    ...
    if post_processing is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol interpretation requires a recorded runtime post-processing bridge.",
        )
    if post_processing.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol interpretation does not match the recorded runtime post-processing bridge.",
        )
    ...
    if materialization is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol interpretation requires a recorded runtime follow-up materialization.",
        )
    if materialization.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol interpretation does not match the recorded runtime follow-up materialization.",
        )
    ...
    if closure is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol interpretation requires a recorded runtime operational closure.",
        )
    if closure.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol interpretation does not match the recorded runtime operational closure.",
        )
    ...
    if intent is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol interpretation requires a recorded runtime protocol execution intent.",
        )
    if intent.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol interpretation does not match the recorded runtime protocol execution intent.",
        )
    ...
    if selection is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol interpretation requires a recorded runtime protocol adapter selection.",
        )
    if selection.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol interpretation does not match the recorded runtime protocol adapter selection.",
        )
    ...
    if dispatch_request is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol interpretation requires a recorded runtime protocol dispatch request.",
        )
    if dispatch_request.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol interpretation does not match the recorded runtime protocol dispatch request.",
        )
    ...
    if invocation_result is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol interpretation requires a recorded runtime protocol invocation result.",
        )
    if invocation_result.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol interpretation does not match the recorded runtime protocol invocation result.",
        )
    ...
    if observation is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol interpretation requires a recorded runtime protocol execution observation.",
        )
    ...
    if observation.invocation_result_record_id != invocation_result.invocation_result_record_id:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution observation does not match the recorded runtime protocol invocation result.",
        )

    _validate_runtime_protocol_interpretation_context(
        execution_metadata=attempt.execution_metadata,
        executor_identifier=payload.executor_identifier,
    )

    result = _build_runtime_protocol_interpretation_result(
        attempt=attempt,
        observation=observation,
        payload=payload,
    )
    interpretation_payload = {"runtime_protocol_interpretation": result.model_dump(mode="json")}
    attempt.execution_metadata = _merge_dicts(attempt.execution_metadata, interpretation_payload)
    command.result_summary = _merge_dicts(command.result_summary, interpretation_payload)
    job_run.result_summary = _merge_dicts(job_run.result_summary, interpretation_payload)
...
def _build_runtime_protocol_interpretation_result(
    *,
    ...
    return RuntimeProtocolInterpretationResult(
        status=RuntimeProtocolInterpretationStatus.RECORDED,
        interpretation_record_id=(
            "runtime-protocol-interpretation:"
            f"{attempt.id}:{observation.observation_record_id}"
        ),
        session_identifier=observation.session_identifier,
        observation_record_id=observation.observation_record_id,
        invocation_result_record_id=observation.invocation_result_record_id,
        dispatch_request_record_id=observation.dispatch_request_record_id,
        selection_record_id=observation.selection_record_id,
        intent_record_id=observation.intent_record_id,
        closure_record_id=observation.closure_record_id,
        materialization_record_id=observation.materialization_record_id,
        post_processing_record_id=observation.post_processing_record_id,
        disposition_record_id=observation.disposition_record_id,
        outcome_record_id=observation.outcome_record_id,
        executor_identifier=payload.executor_identifier,
        ...
        downstream_state=observation.downstream_state,
        interpretation_key="placeholder.protocol.runtime-meaning",
        interpretation_family=(
            RuntimeProtocolInterpretationFamily.PLACEHOLDER_PROTOCOL_TERMINAL_INTERPRETATION
        ),
        interpretation_payload=RuntimeProtocolInterpretationPayload(
            schema_version="placeholder-runtime-protocol-interpretation.v1",
            interpretation_state=(
                RuntimeProtocolInterpretationState.PLACEHOLDER_RUNTIME_INTERPRETATION_RECORDED
            ),
            runtime_meaning_mode=observation_payload.observation_mode,
            adapter_family=observation_payload.adapter_family,
            capability_profile=observation_payload.capability_profile,
            semantic_outcome_classification=(
                RuntimeProtocolSemanticOutcomeClassification.PLACEHOLDER_TERMINAL_MEANING_READY
            ),
        ...
        included_follow_up_descriptor_types=observation.included_follow_up_descriptor_types,
        interpretation_recorded_at=datetime.now(UTC).isoformat(),
        interpretation_recorded_by_executor_identifier=payload.executor_identifier,
        interpretation_reason=payload.interpretation_reason,
        already_recorded=False,
        summary=(
            "Runtime protocol execution observation is converted into a durable placeholder "
            "runtime-facing terminal interpretation without executing any live protocol meaning bridge."
        ),
        lineage=observation.lineage,
    )


def _validate_runtime_protocol_interpretation_context(
    *,
    ...
    if lease is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol interpretation requires a runtime lease.",
        )
    ...
    if invocation is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol interpretation requires a runtime invocation gate.",
        )
    ...
    if guard is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol interpretation requires a runtime execution guard.",
        )
