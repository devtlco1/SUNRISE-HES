...
def test_redis_worker_exception_governance_maps_into_stable_worker_remediation_records_correctly(
    client,
    db_session: Session,
) -> None:
    ...
    remediation = map_worker_exception_governance_to_remediation(exception_governance)

    assert remediation.records[0].remediation_type == "noop_remediation"
    assert remediation.records[1].remediation_type == "remediation_summary"
    assert remediation.records[2].remediation_type == "response_action_record"
    assert remediation.records[3].remediation_type == "remediation_note"
    assert remediation.records[4].remediation_type == "exception_response_artifact"
    assert remediation.total_records == 5
    assert remediation.terminal_records == 4
    assert remediation.response_ready is True


def test_remediation_response_note_and_exception_artifacts_are_exposed_correctly(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
    ...
    remediation_values = [record.remediation_type for record in remediation.records]
    assert remediation_values == [
        "noop_remediation",
        "remediation_summary",
        "response_action_record",
        "remediation_note",
        "exception_response_artifact",
    ]


def test_lineage_and_correlation_are_preserved_through_worker_remediation_normalization(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
    ...
    assert remediation.metadata["source_identifiers"]["job_run_id"] == job_run_id
    assert remediation.metadata["source_identifiers"]["command_id"] is not None
    assert remediation.metadata["correlation_lineage"]["derived_correlation_id"] == dispatch_request.derived_correlation_id
...
def test_repeated_enqueue_calls_stay_idempotent_with_worker_remediation_generation(
    client,
    db_session: Session,
    monkeypatch,
) -> None:
...
def test_summaries_remain_consistent_after_worker_remediation_generation(
    client,
    db_session: Session,
    monkeypatch,
) -> None:
    ...
    remediation = job_run.result_summary["queue_enqueue"]["enqueue_metadata"]["redis_worker_remediation"]
    assert remediation["records"][0]["remediation_type"] == "noop_remediation"
    assert remediation["records"][1]["remediation_type"] == "remediation_summary"
    assert remediation["records"][3]["remediation_type"] == "remediation_note"
    assert remediation["total_records"] == 5
