...
def test_redis_worker_closure_archive_maps_into_stable_worker_retention_register_records_correctly(
    client,
    db_session: Session,
) -> None:
    ...
    closure_archive = map_worker_ledger_closure_to_closure_archive(
        map_worker_final_attestation_ledger_to_ledger_closure(
            map_worker_attestation_seal_to_final_attestation_ledger(
                map_worker_completion_attestation_to_attestation_seal(
                    map_worker_response_certification_to_completion_attestation(
    ...
    register = map_worker_closure_archive_to_retention_register(closure_archive)

    assert register.records[0].register_type == "noop_retention_register"
    assert register.records[1].register_type == "retention_register_summary"
    assert register.records[2].register_type == "register_action_record"
    assert register.records[3].register_type == "retention_register_note"
    assert register.records[4].register_type == "final_register_artifact"
    assert register.total_records == 5
    assert register.terminal_records == 4
    assert register.register_ready is True


def test_retention_register_action_note_and_final_register_artifacts_are_exposed_correctly(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
    ...
    dispatch_request = get_dispatch_request_projection(db_session, job_run_id=job_run_id)
    payload = build_queue_enqueue_payload(dispatch_request)
    register = map_worker_closure_archive_to_retention_register(
        map_worker_ledger_closure_to_closure_archive(
            map_worker_final_attestation_ledger_to_ledger_closure(
                map_worker_attestation_seal_to_final_attestation_ledger(
                    map_worker_completion_attestation_to_attestation_seal(
    ...
    register_values = [record.register_type for record in register.records]
    assert register_values == [
        "noop_retention_register",
        "retention_register_summary",
        "register_action_record",
        "retention_register_note",
        "final_register_artifact",
    ]


def test_lineage_and_correlation_are_preserved_through_worker_retention_register_normalization(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
    ...
    assert register.metadata["source_identifiers"]["job_run_id"] == job_run_id
    assert register.metadata["source_identifiers"]["command_id"] is not None
    assert register.metadata["correlation_lineage"]["derived_correlation_id"] == dispatch_request.derived_correlation_id
...
def test_repeated_enqueue_calls_stay_idempotent_with_worker_retention_register_generation(
    client,
    db_session: Session,
    monkeypatch,
) -> None:
...
def test_summaries_remain_consistent_after_worker_retention_register_generation(
    client,
    db_session: Session,
    monkeypatch,
) -> None:
    ...
    register = job_run.result_summary["queue_enqueue"]["enqueue_metadata"][
