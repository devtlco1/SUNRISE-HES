...
def bridge_runtime_delivery_contract_to_dispatch_envelope(
    session: Session,
    *,
    ...
    if attempt.job_run_id is None:
        raise HTTPException(
            ...
            detail="Runtime dispatch envelope requires an attempt linked to a job run.",
        )
    ...
    existing = _load_runtime_dispatch_envelope(attempt.execution_metadata)
    if existing is not None:
        if existing.session_identifier != payload.session_identifier:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Runtime dispatch envelope is already recorded for another session.",
            )
        if existing.executor_identifier != payload.executor_identifier:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Runtime dispatch envelope is already owned by another executor.",
            )
        return RuntimeDispatchEnvelopeBridgeResponse(
    ...
    if session_result is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope requires a finalized runtime session.",
        )
    if session_result.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope does not match the finalized runtime session.",
        )
    ...
    if outcome is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope requires a recorded runtime execution outcome.",
        )
    if outcome.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope does not match the recorded runtime execution outcome.",
        )
    ...
    if disposition is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope requires a recorded runtime attempt disposition.",
        )
    if disposition.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope does not match the recorded runtime attempt disposition.",
        )
    ...
    if post_processing is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope requires a recorded runtime post-processing bridge.",
        )
    if post_processing.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope does not match the recorded runtime post-processing bridge.",
        )
    ...
    if materialization is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope requires a recorded runtime follow-up materialization.",
        )
    if materialization.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope does not match the recorded runtime follow-up materialization.",
        )
    ...
    if closure is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope requires a recorded runtime operational closure.",
        )
    if closure.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope does not match the recorded runtime operational closure.",
        )
    ...
    if intent is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope requires a recorded runtime protocol execution intent.",
        )
    if intent.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope does not match the recorded runtime protocol execution intent.",
        )
    ...
    if selection is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope requires a recorded runtime protocol adapter selection.",
        )
    if selection.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope does not match the recorded runtime protocol adapter selection.",
        )
    ...
    if dispatch_request is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope requires a recorded runtime protocol dispatch request.",
        )
    if dispatch_request.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope does not match the recorded runtime protocol dispatch request.",
        )
    ...
    if invocation_result is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope requires a recorded runtime protocol invocation result.",
        )
    if invocation_result.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope does not match the recorded runtime protocol invocation result.",
        )
    ...
    if observation is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope requires a recorded runtime protocol execution observation.",
        )
    if observation.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope does not match the recorded runtime protocol execution observation.",
        )
    ...
    if interpretation is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope requires a recorded runtime protocol interpretation.",
        )
    if interpretation.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope does not match the recorded runtime protocol interpretation.",
        )
    ...
    if reconciliation is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope requires a recorded runtime protocol reconciliation.",
        )
    if reconciliation.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope does not match the recorded runtime protocol reconciliation.",
        )
    ...
    if settlement is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope requires a recorded runtime terminal settlement.",
        )
    if settlement.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope does not match the recorded runtime terminal settlement.",
        )
    ...
    if attestation is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope requires a recorded runtime closure attestation.",
        )
    if attestation.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope does not match the recorded runtime closure attestation.",
        )
    ...
    if publication_contract is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope requires a recorded runtime publication contract.",
        )
    if publication_contract.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope does not match the recorded runtime publication contract.",
        )
    ...
    if externalization_envelope is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope requires a recorded runtime externalization envelope.",
        )
    if externalization_envelope.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope does not match the recorded runtime externalization envelope.",
        )
    ...
    if delivery_contract is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope requires a recorded runtime delivery contract.",
        )
    ...
    if (
        delivery_contract.envelope_record_id
        != externalization_envelope.envelope_record_id
    ):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract does not match the recorded runtime externalization envelope.",
        )

    _validate_runtime_dispatch_envelope_context(
        execution_metadata=attempt.execution_metadata,
        executor_identifier=payload.executor_identifier,
    )

    result = _build_runtime_dispatch_envelope_result(
        attempt=attempt,
        delivery_contract=delivery_contract,
        payload=payload,
    )
    dispatch_payload = {"runtime_dispatch_envelope": result.model_dump(mode="json")}
    ...
    job_run.result_summary = merge_runtime_metadata(job_run.result_summary, dispatch_payload)
...
def _build_runtime_dispatch_envelope_result(
    *,
    attempt: CommandExecutionAttempt,
    delivery_contract: RuntimeDeliveryContractResult,
    payload: RuntimeDispatchEnvelopeBridgeRequest,
) -> RuntimeDispatchEnvelopeResult:
    delivery_payload = delivery_contract.delivery_contract_payload
    return RuntimeDispatchEnvelopeResult(
        status=RuntimeDispatchEnvelopeStatus.RECORDED,
        dispatch_envelope_record_id=(
            "runtime-dispatch-envelope:"
            f"{attempt.id}:{delivery_contract.delivery_contract_record_id}"
        ),
        session_identifier=delivery_contract.session_identifier,
        delivery_contract_record_id=delivery_contract.delivery_contract_record_id,
        envelope_record_id=delivery_contract.envelope_record_id,
        publication_contract_record_id=delivery_contract.publication_contract_record_id,
        attestation_record_id=delivery_contract.attestation_record_id,
        settlement_record_id=delivery_contract.settlement_record_id,
        reconciliation_record_id=delivery_contract.reconciliation_record_id,
        interpretation_record_id=delivery_contract.interpretation_record_id,
        observation_record_id=delivery_contract.observation_record_id,
        invocation_result_record_id=delivery_contract.invocation_result_record_id,
        dispatch_request_record_id=delivery_contract.dispatch_request_record_id,
        selection_record_id=delivery_contract.selection_record_id,
        intent_record_id=delivery_contract.intent_record_id,
        closure_record_id=delivery_contract.closure_record_id,
        materialization_record_id=delivery_contract.materialization_record_id,
        post_processing_record_id=delivery_contract.post_processing_record_id,
        disposition_record_id=delivery_contract.disposition_record_id,
        outcome_record_id=delivery_contract.outcome_record_id,
        executor_identifier=payload.executor_identifier,
        job_run_id=str(attempt.job_run_id),
        related_command_id=str(attempt.meter_command_id),
        command_attempt_id=str(attempt.id),
        terminal_outcome=delivery_contract.terminal_outcome,
        downstream_state=delivery_contract.downstream_state,
        dispatch_envelope_key="placeholder.runtime.dispatch-envelope",
        dispatch_envelope_family=(
            RuntimeDispatchEnvelopeFamily.PLACEHOLDER_RUNTIME_DISPATCH_ENVELOPE
        ),
        dispatch_envelope_payload=RuntimeDispatchEnvelopePayload(
            schema_version="placeholder-runtime-dispatch-envelope.v1",
            dispatch_envelope_state=(
                RuntimeDispatchEnvelopeState.PLACEHOLDER_HANDOFF_READINESS_RECORDED
            ),
            handoff_readiness_mode=delivery_payload.dispatch_readiness_mode,
            adapter_family=delivery_payload.adapter_family,
            capability_profile=delivery_payload.capability_profile,
            dispatch_classification=(
                RuntimeDispatchEnvelopeClassification.PLACEHOLDER_RUNTIME_DISPATCH_ENVELOPE_READY
            ),
            outbound_channel_family=(
                RuntimeDispatchOutboundChannelFamily.PLACEHOLDER_OUTBOUND_DELIVERY_CHANNEL
            ),
            placeholder_chain_references={
                "delivery_contract_record_id": (
                    delivery_contract.delivery_contract_record_id
                ),
                "envelope_record_id": delivery_contract.envelope_record_id,
                "publication_contract_record_id": (
                    delivery_contract.publication_contract_record_id
                ),
                "attestation_record_id": delivery_contract.attestation_record_id,
                "settlement_record_id": delivery_contract.settlement_record_id,
                "reconciliation_record_id": delivery_contract.reconciliation_record_id,
        ...
        included_follow_up_descriptor_types=delivery_contract.included_follow_up_descriptor_types,
        dispatch_envelope_recorded_at=datetime.now(UTC).isoformat(),
        dispatch_envelope_recorded_by_executor_identifier=payload.executor_identifier,
        dispatch_envelope_reason=payload.dispatch_envelope_reason,
        already_recorded=False,
        summary=(
            "Runtime delivery contract is converted into a durable placeholder "
            "outbound dispatch envelope without executing any live publication or delivery logic."
        ),
        lineage=delivery_contract.lineage,
    )


def _validate_runtime_dispatch_envelope_context(
    *,
    ...
    if lease is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope requires a runtime lease.",
        )
    ...
    if invocation is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope requires a runtime invocation gate.",
        )
    ...
    if guard is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime dispatch envelope requires a runtime execution guard.",
        )
