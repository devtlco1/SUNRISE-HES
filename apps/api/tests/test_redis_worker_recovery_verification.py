...
def test_redis_worker_remediation_maps_into_stable_worker_recovery_verification_records_correctly(
    client,
    db_session: Session,
) -> None:
    ...
    remediation = map_worker_exception_governance_to_remediation(
        map_worker_oversight_to_exception_governance(
    ...
    verification = map_worker_remediation_to_recovery_verification(remediation)

    assert verification.records[0].verification_type == "noop_recovery_verification"
    assert verification.records[1].verification_type == "recovery_verification_summary"
    assert verification.records[2].verification_type == "verification_action_record"
    assert verification.records[3].verification_type == "recovery_confirmation_note"
    assert verification.records[4].verification_type == "response_confirmation_artifact"
    assert verification.total_records == 5
    assert verification.terminal_records == 4
    assert verification.confirmation_ready is True


def test_recovery_verification_action_note_and_response_confirmation_are_exposed_correctly(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
    ...
    dispatch_request = get_dispatch_request_projection(db_session, job_run_id=job_run_id)
    payload = build_queue_enqueue_payload(dispatch_request)
    verification = map_worker_remediation_to_recovery_verification(
    ...
    verification_values = [record.verification_type for record in verification.records]
    assert verification_values == [
        "noop_recovery_verification",
        "recovery_verification_summary",
        "verification_action_record",
        "recovery_confirmation_note",
        "response_confirmation_artifact",
    ]


def test_lineage_and_correlation_are_preserved_through_worker_recovery_verification_normalization(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
    ...
    assert verification.metadata["source_identifiers"]["job_run_id"] == job_run_id
    assert verification.metadata["source_identifiers"]["command_id"] is not None
    assert verification.metadata["correlation_lineage"]["derived_correlation_id"] == dispatch_request.derived_correlation_id
...
def test_repeated_enqueue_calls_stay_idempotent_with_worker_recovery_verification_generation(
    client,
    db_session: Session,
    monkeypatch,
) -> None:
...
def test_summaries_remain_consistent_after_worker_recovery_verification_generation(
    client,
    db_session: Session,
    monkeypatch,
) -> None:
    ...
    verification = job_run.result_summary["queue_enqueue"]["enqueue_metadata"]["redis_worker_recovery_verification"]
    assert verification["records"][0]["verification_type"] == "noop_recovery_verification"
    assert verification["records"][1]["verification_type"] == "recovery_verification_summary"
    assert verification["records"][3]["verification_type"] == "recovery_confirmation_note"
    assert verification["total_records"] == 5
