...
from app.modules.commands.enums import CommandCategory, CommandStatus
...
from app.modules.commands.schemas import (
    ...
    ConsumeQueuedProfileCaptureExecutionResult,
    ...
    ProfileCaptureQueuedExecutionEnqueueResult,
    ProfileCaptureQueuedExecutionLease,
    ProfileCaptureQueuedExecutionMessage,
    ProfileCaptureQueuedExecutionMessageSource,
)
...
PROFILE_CAPTURE_QUEUE_CONTRACT_FAMILY = "profile_capture_queued_execution"
PROFILE_CAPTURE_QUEUE_CONTRACT_VERSION = "v1"
PROFILE_CAPTURE_QUEUE_DISPATCH_CATEGORY = "profile_capture_queued_execution"
PROFILE_CAPTURE_QUEUE_INTENDED_WORKER_PATH = "commands.profile_capture.queue_worker"
PROFILE_CAPTURE_QUEUE_ROUTING_KEY = "commands.profile_capture.queued_execution"
PROFILE_CAPTURE_QUEUE_EXECUTOR_PREFIX = "profile_capture_queue_worker"
PROFILE_CAPTURE_QUEUE_OPERATION = "capture_load_profile"


def enqueue_profile_capture_command_execution(
    session: Session,
    *,
    ...
    existing_artifact = _load_profile_capture_queue_enqueue_artifact(command.result_summary)
    ...
    _assert_profile_capture_queue_enqueue_eligible(session, command=command)
    queue_message = _build_profile_capture_queue_message(command=command, payload=payload)
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail="Redis queue backend is unavailable for profile-capture enqueue.",
        ) from exc

    enqueued_at = datetime.now(UTC)
    apply_command_status_transition(command, new_status=CommandStatus.QUEUED, now=enqueued_at)
    queue_artifact = _build_profile_capture_queue_enqueue_artifact(
        command=command,
        queue_message=queue_message,
        enqueue_identifier=payload.enqueue_identifier,
    ...
    command.result_summary = merge_runtime_metadata(
        ...
        {"profile_capture_queue_enqueue": queue_artifact},
    )
    session.add(command)
    session.commit()
    session.refresh(command)
    return ProfileCaptureQueuedExecutionEnqueueResponse(
        result=ProfileCaptureQueuedExecutionEnqueueResult(
            ...
            profile_read_operation=queue_message.profile_read_operation,
            reused_existing_enqueue=False,
            enqueued_at=enqueued_at,
...
def consume_next_queued_profile_capture_execution(
    session: Session,
    *,
    ...
    if claim_result.message is None or claim_result.claim is None or claim_result.dequeue is None:
        return ConsumeQueuedProfileCaptureExecutionResponse(
    ...
    queue_message = _validate_claimed_profile_capture_queue_message(claim_result.message)
    command = get_meter_command(session, queue_message.source.command_id)
    enqueue_artifact = _load_profile_capture_queue_enqueue_artifact(command.result_summary)
    if enqueue_artifact is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Profile-capture command is missing the durable queue enqueue artifact.",
        )
    if enqueue_artifact.get("enqueue_identifier") != queue_message.enqueue_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Claimed profile-capture queue message does not match the durable enqueue artifact.",
        )
    ...
    orchestration = orchestrate_profile_capture_command_execution(
        ...
        payload=ProfileCaptureExecutionOrchestrationRequest(
            ...
            orchestration_reason=payload.consume_reason or "profile-capture-queued-execution",
    ...
    if attempt is None or refreshed_command is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Profile-capture queued worker execution could not load durable execution state.",
        )

    status_response = get_profile_capture_execution_status(session, command_id=refreshed_command.id)
    ...
    queue_consumption_record = _build_profile_capture_queue_consumption_record(
        ...
        worker_identifier=payload.worker_identifier,
        terminal_status_category=status_response.result.terminal_status_category,
        runtime_profile_read_execution_record_id=(
            status_response.result.runtime_profile_read_execution_record_id
        ),
        ack_receipt_id=ack_result.ack_receipt_id,
        acked_at=ack_result.acked_at,
        consumed_at=consumed_at,
    )
    artifact_payload = {"profile_capture_queue_consumption": queue_consumption_record}
...
def _build_existing_enqueue_response(
    *,
    ...
    if artifact.get("enqueue_identifier") != payload.enqueue_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Profile-capture command already has a queued execution owned by another enqueue identifier.",
        )
    queue_message = ProfileCaptureQueuedExecutionMessage.model_validate(artifact["queue_message"])
...
def _assert_profile_capture_queue_enqueue_eligible(
    session: Session,
    *,
    command: MeterCommand,
) -> None:
    if command.command_template.category != CommandCategory.PROFILE_CAPTURE:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Selected command is not compatible with the profile-capture queued execution slice.",
        )
    if command.current_status not in {
        ...
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Profile-capture command is not queue-eligible from its current state.",
        )
    ...
    if latest_attempt is not None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Profile-capture command already has execution attempt history and cannot enter the bounded queued foundation.",
        )
    _validate_capture_load_profile_normalized_payload(command)


def _build_profile_capture_queue_message(
    *,
    command: MeterCommand,
    payload: ProfileCaptureQueuedExecutionEnqueueRequest,
) -> ProfileCaptureQueuedExecutionMessage:
    normalized_payload = _validate_capture_load_profile_normalized_payload(command)
    capture_load_profile = normalized_payload["capture_load_profile"]
    channel_ids = [uuid.UUID(str(item)) for item in capture_load_profile["channel_ids"]]
    return ProfileCaptureQueuedExecutionMessage(
        contract_family=PROFILE_CAPTURE_QUEUE_CONTRACT_FAMILY,
        contract_version=PROFILE_CAPTURE_QUEUE_CONTRACT_VERSION,
        enqueue_identifier=payload.enqueue_identifier,
        command_category=CommandCategory.PROFILE_CAPTURE,
        profile_read_operation=str(normalized_payload["profile_read_operation"]),
        interval_start=datetime.fromisoformat(str(capture_load_profile["interval_start"])),
        interval_end=datetime.fromisoformat(str(capture_load_profile["interval_end"])),
        channel_ids=channel_ids,
        channel_count=len(channel_ids),
        intended_worker_path=PROFILE_CAPTURE_QUEUE_INTENDED_WORKER_PATH,
...
def _build_dispatch_request_identity(
    *,
    ...
    return f"profile-capture-queue:{command_id}:{enqueue_identifier}"
...
def _build_redis_stream_fields(
    *,
    ...
    return {
        "backend_name": "redis",
        "message_type": PROFILE_CAPTURE_QUEUE_CONTRACT_FAMILY,
        "payload_version": PROFILE_CAPTURE_QUEUE_CONTRACT_VERSION,
        "dispatch_category": PROFILE_CAPTURE_QUEUE_DISPATCH_CATEGORY,
        "dispatch_request_identity": dispatch_request_identity,
        "routing_key": PROFILE_CAPTURE_QUEUE_ROUTING_KEY,
        ...
        "dispatch_metadata": json.dumps(
            {
                "contract_family": PROFILE_CAPTURE_QUEUE_CONTRACT_FAMILY,
                "contract_version": PROFILE_CAPTURE_QUEUE_CONTRACT_VERSION,
                "command_category": CommandCategory.PROFILE_CAPTURE.value,
                "profile_read_operation": queue_message.profile_read_operation,
                "channel_count": queue_message.channel_count,
            },
            sort_keys=True,
        ),
        "intended_worker_path": PROFILE_CAPTURE_QUEUE_INTENDED_WORKER_PATH,
...
def _build_profile_capture_queue_enqueue_artifact(
    *,
    command: MeterCommand,
    queue_message: ProfileCaptureQueuedExecutionMessage,
    enqueue_identifier: str,
...
def _validate_claimed_profile_capture_queue_message(
    claimed_message,
) -> ProfileCaptureQueuedExecutionMessage:
    if claimed_message.dispatch_category != PROFILE_CAPTURE_QUEUE_DISPATCH_CATEGORY:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Claimed dispatch message is not compatible with the profile-capture queued execution slice.",
        )
    if claimed_message.intended_worker_path != PROFILE_CAPTURE_QUEUE_INTENDED_WORKER_PATH:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Claimed dispatch message is not routed to the profile-capture queue worker path.",
        )
    queue_message = ProfileCaptureQueuedExecutionMessage.model_validate(claimed_message.body)
    if queue_message.contract_family != PROFILE_CAPTURE_QUEUE_CONTRACT_FAMILY:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Claimed dispatch message is not owned by the profile-capture queued execution contract.",
        )
    if queue_message.contract_version != PROFILE_CAPTURE_QUEUE_CONTRACT_VERSION:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Claimed profile-capture queue message uses an unsupported contract version.",
        )
    if queue_message.command_category != CommandCategory.PROFILE_CAPTURE:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Claimed queue message is not for a profile-capture command.",
        )
    if queue_message.profile_read_operation != PROFILE_CAPTURE_QUEUE_OPERATION:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Claimed queue message is not for the bounded capture-load-profile operation.",
        )
    if queue_message.channel_count <= 0 or not queue_message.channel_ids:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Claimed queue message is missing bounded capture-load-profile channel linkage.",
        )
    return queue_message


def _build_queue_orchestration_identifier(enqueue_identifier: str) -> str:
    return f"pc-queue:{uuid.uuid5(uuid.NAMESPACE_URL, enqueue_identifier)}"


def _build_queue_executor_identifier(worker_identifier: str) -> str:
    return f"{PROFILE_CAPTURE_QUEUE_EXECUTOR_PREFIX}:{worker_identifier}"


def _build_queue_lease(*, claim_result) -> ProfileCaptureQueuedExecutionLease:
    ...
    return ProfileCaptureQueuedExecutionLease(
...
def _build_profile_capture_queue_consumption_record(
    *,
    command: MeterCommand,
    attempt: CommandExecutionAttempt,
    job_run: JobRun | None,
    queue_message: ProfileCaptureQueuedExecutionMessage,
    queue_lease: ProfileCaptureQueuedExecutionLease,
    worker_identifier: str,
    terminal_status_category: str | None,
    runtime_profile_read_execution_record_id: str | None,
    ack_receipt_id: str,
    acked_at: str,
...
def _load_profile_capture_queue_enqueue_artifact(
    result_summary: dict[str, object] | None,
) -> dict[str, object] | None:
    if not isinstance(result_summary, dict):
        return None
    payload = result_summary.get("profile_capture_queue_enqueue")
