...
def test_redis_worker_finalization_timeline_maps_into_stable_worker_closure_report_records_correctly(
    client,
    db_session: Session,
) -> None:
    ...
    report = map_worker_finalization_to_closure_report(finalizations)

    assert report.records[0].report == "noop_closure_report"
    assert report.records[1].report == "broker_closure_summary"
    assert report.records[2].report == "reconciliation_ready_report"
    assert report.records[3].report == "queue_observability_report"
    assert report.records[4].report == "retention_summary"
    assert report.total_records == 5
    assert report.terminal_records == 4


def test_broker_reconciliation_observability_and_retention_reports_are_exposed_correctly(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
    ...
    dispatch_request = get_dispatch_request_projection(db_session, job_run_id=job_run_id)
    payload = build_queue_enqueue_payload(dispatch_request)
    report = map_worker_finalization_to_closure_report(
    ...
    assert report_values == [
        "noop_closure_report",
        "broker_closure_summary",
        "reconciliation_ready_report",
        "queue_observability_report",
        "retention_summary",
    ]


def test_lineage_and_correlation_are_preserved_through_worker_closure_report_normalization(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
    ...
    assert report.metadata["source_identifiers"]["job_run_id"] == job_run_id
    assert report.metadata["source_identifiers"]["command_id"] is not None
    assert report.metadata["correlation_lineage"]["derived_correlation_id"] == dispatch_request.derived_correlation_id
...
def test_repeated_enqueue_calls_stay_idempotent_with_worker_closure_report_generation(
    client,
    db_session: Session,
    monkeypatch,
) -> None:
...
def test_summaries_remain_consistent_after_worker_closure_report_generation(
    client,
    db_session: Session,
    monkeypatch,
) -> None:
    ...
    report = job_run.result_summary["queue_enqueue"]["enqueue_metadata"]["redis_worker_closure_report"]
    assert report["records"][0]["report"] == "noop_closure_report"
    assert report["records"][1]["report"] == "broker_closure_summary"
    assert report["records"][3]["report"] == "queue_observability_report"
    assert report["total_records"] == 5
