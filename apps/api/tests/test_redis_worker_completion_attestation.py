...
def test_redis_worker_response_certification_maps_into_stable_worker_completion_attestation_records_correctly(
    client,
    db_session: Session,
) -> None:
    ...
    response_certification = map_worker_response_assurance_to_response_certification(
        map_worker_recovery_verification_to_response_assurance(
            map_worker_remediation_to_recovery_verification(
    ...
    attestation = map_worker_response_certification_to_completion_attestation(
        response_certification
    )

    assert attestation.records[0].attestation_type == "noop_completion_attestation"
    assert attestation.records[1].attestation_type == "completion_attestation_summary"
    assert attestation.records[2].attestation_type == "attestation_action_record"
    assert attestation.records[3].attestation_type == "attestation_note"
    assert attestation.records[4].attestation_type == "completion_proof_artifact"
    assert attestation.total_records == 5
    assert attestation.terminal_records == 4
    assert attestation.attestation_ready is True


def test_completion_attestation_action_note_and_completion_proof_artifacts_are_exposed_correctly(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
    ...
    attestation_values = [record.attestation_type for record in attestation.records]
    assert attestation_values == [
        "noop_completion_attestation",
        "completion_attestation_summary",
        "attestation_action_record",
        "attestation_note",
        "completion_proof_artifact",
    ]


def test_lineage_and_correlation_are_preserved_through_worker_completion_attestation_normalization(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
    ...
    assert attestation.metadata["source_identifiers"]["job_run_id"] == job_run_id
    assert attestation.metadata["source_identifiers"]["command_id"] is not None
    assert (
        attestation.metadata["correlation_lineage"]["derived_correlation_id"]
...
def test_repeated_enqueue_calls_stay_idempotent_with_worker_completion_attestation_generation(
    client,
    db_session: Session,
    monkeypatch,
) -> None:
...
def test_summaries_remain_consistent_after_worker_completion_attestation_generation(
    client,
    db_session: Session,
    monkeypatch,
) -> None:
    ...
    attestation = job_run.result_summary["queue_enqueue"]["enqueue_metadata"][
