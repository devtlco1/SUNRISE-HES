...
from app.modules.commands.schemas import (
    ...
    RelayControlRuntimeTerminalizationResult,
)
...
def terminalize_relay_control_runtime_execution(
    session: Session,
    *,
    ...
    existing_terminalization = _load_relay_control_runtime_terminalization(
    ...
    if command.command_template.category not in {
        ...
        raise HTTPException(
            ...
            detail="Selected command is not compatible with the relay-control runtime terminalization slice.",
        )
    if attempt is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Relay-control command does not have a runtime-backed attempt for terminalization.",
        )

    handoff_record = _load_relay_control_runtime_handoff(attempt.execution_metadata)
    if handoff_record is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Relay-control attempt is missing the runtime handoff artifact required for terminalization.",
        )
    runtime_execution = _load_runtime_relay_control_execution(attempt.execution_metadata)
    if runtime_execution is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Relay-control attempt is not yet terminalizable because the runtime relay-control execution artifact is missing.",
        )
    runtime_outcome = _load_runtime_execution_outcome(attempt.execution_metadata)
    if runtime_outcome is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Relay-control attempt is not yet terminalizable because the runtime execution outcome artifact is missing.",
        )
    _validate_relay_control_terminalization_context(
        command=command,
        attempt=attempt,
    ...
    terminalization_record = _build_relay_control_runtime_terminalization_record(
        command=command,
        attempt=attempt,
        runtime_execution=runtime_execution,
        payload=payload,
    ...
    attempt.execution_metadata = merge_runtime_metadata(
        ...
        {"relay_control_runtime_terminalization": terminalization_record},
    )
    result_summary = merge_runtime_metadata(
        command.result_summary,
        {"relay_control_runtime_terminalization": terminalization_record},
    )
    _apply_relay_control_terminal_state(
        attempt=attempt,
        command=command,
...
def _build_existing_terminalization_response(
    ...
    *,
    ...
    if attempt is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Relay-control runtime terminalization artifact references a missing attempt.",
        )
    if terminalization_record.get("terminalization_identifier") != payload.terminalization_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Relay-control runtime terminalization already exists for another terminalization identifier.",
        )
    if terminalization_record.get("executor_identifier") != payload.executor_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Relay-control runtime terminalization is already owned by another executor.",
        )
    job_run = session.get(JobRun, attempt.job_run_id) if attempt.job_run_id is not None else None
    return _build_terminalization_response(
        command=command,
...
def _validate_relay_control_terminalization_context(
    *,
    command: MeterCommand,
    attempt: CommandExecutionAttempt,
    handoff_record: dict[str, object],
    runtime_execution: RuntimeRelayControlExecutionResult,
    runtime_outcome: RuntimeExecutionOutcomeResult,
    executor_identifier: str,
) -> None:
    if attempt.job_run_id is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Relay-control attempt is not linked to a job run for terminalization.",
        )
    if handoff_record.get("handoff_status") != "handed_off":
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Relay-control runtime handoff artifact is invalid for terminalization.",
        )
    if handoff_record.get("executor_identifier") != executor_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Relay-control runtime handoff is owned by another executor.",
        )
    if runtime_execution.command_category not in {
        CommandCategory.REMOTE_DISCONNECT,
        CommandCategory.REMOTE_RECONNECT,
    }:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime relay-control execution is incompatible with relay-control terminalization.",
        )
    if str(runtime_execution.related_command_id) != str(command.id):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime relay-control execution linkage is inconsistent with the related command.",
        )
    if str(runtime_execution.command_attempt_id) != str(attempt.id):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime relay-control execution linkage is inconsistent with the command attempt.",
        )
    if str(runtime_execution.job_run_id) != str(attempt.job_run_id):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime relay-control execution linkage is inconsistent with the job run.",
        )
    if runtime_execution.relay_control_recorded_by_executor_identifier != executor_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime relay-control execution is owned by another executor.",
        )
    if runtime_execution.status != RuntimeRelayControlExecutionStatus.COMPLETED:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Relay-control runtime execution is not yet terminalizable.",
        )
    if str(runtime_outcome.related_command_id) != str(command.id):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime execution outcome linkage is inconsistent with the related command.",
        )
    if str(runtime_outcome.command_attempt_id) != str(attempt.id):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime execution outcome linkage is inconsistent with the command attempt.",
        )
    if str(runtime_outcome.job_run_id) != str(attempt.job_run_id):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime execution outcome linkage is inconsistent with the job run.",
        )
    if runtime_outcome.session_identifier != runtime_execution.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime execution outcome linkage is inconsistent with the runtime session context.",
        )
    if runtime_outcome.outcome_recorded_by_executor_identifier != executor_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
    ...
    if (
        ...
        != runtime_execution.relay_control_execution_record_id
    ):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime handoff artifact is inconsistent with the recorded runtime relay-control execution.",
        )
    ...
    *,
    ...
    if runtime_execution.execution_outcome == RuntimeCommandOutcome.TIMED_OUT:
        return {
            ...
            "error_code": "RELAY_CONTROL_RUNTIME_TIMED_OUT",
    ...
    return {
        ...
        "error_code": f"RELAY_CONTROL_{runtime_execution.execution_outcome.value.upper()}",
        "error_message": runtime_execution.error_detail or runtime_execution.summary,
    }


def _build_relay_control_runtime_terminalization_record(
    *,
    command: MeterCommand,
    attempt: CommandExecutionAttempt,
    runtime_execution: RuntimeRelayControlExecutionResult,
    ...
    return {
        ...
        "executor_identifier": payload.executor_identifier,
        "runtime_relay_control_execution_record_id": runtime_execution.relay_control_execution_record_id,
        "relay_control_operation": runtime_execution.relay_operation.value,
        "relay_control_execution_outcome": runtime_execution.execution_outcome.value,
...
def _apply_relay_control_terminal_state(
    *,
    attempt: CommandExecutionAttempt,
    command: MeterCommand,
    job_run: JobRun | None,
...
def _build_terminalization_response(
    *,
    ...
    return RelayControlRuntimeTerminalizationResponse(
        result=RelayControlRuntimeTerminalizationResult(
            ...
            runtime_relay_control_execution_record_id=str(
                terminalization_record["runtime_relay_control_execution_record_id"]
            ),
            relay_control_operation=RelayControlCommandOperation(
                str(terminalization_record["relay_control_operation"])
            ),
            relay_control_execution_outcome=str(
                terminalization_record["relay_control_execution_outcome"]
            ),
...
def _load_relay_control_runtime_terminalization(
    execution_metadata: dict[str, object] | None,
) -> dict[str, object] | None:
    if not isinstance(execution_metadata, dict):
        return None
    payload = execution_metadata.get("relay_control_runtime_terminalization")
    return payload if isinstance(payload, dict) else None
