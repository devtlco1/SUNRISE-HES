...
def test_redis_worker_attestation_seal_maps_into_stable_worker_final_attestation_ledger_records_correctly(
    client,
    db_session: Session,
) -> None:
    ...
    attestation_seal = map_worker_completion_attestation_to_attestation_seal(
        map_worker_response_certification_to_completion_attestation(
            map_worker_response_assurance_to_response_certification(
                map_worker_recovery_verification_to_response_assurance(
                    map_worker_remediation_to_recovery_verification(
    ...
    ledger = map_worker_attestation_seal_to_final_attestation_ledger(attestation_seal)

    assert ledger.records[0].ledger_type == "noop_final_attestation_ledger"
    assert ledger.records[1].ledger_type == "final_attestation_ledger_summary"
    assert ledger.records[2].ledger_type == "ledger_action_record"
    assert ledger.records[3].ledger_type == "notarization_note"
    assert ledger.records[4].ledger_type == "final_ledger_artifact"
    assert ledger.total_records == 5
    assert ledger.terminal_records == 4
    assert ledger.ledger_ready is True


def test_final_attestation_ledger_action_note_and_final_ledger_artifacts_are_exposed_correctly(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
    ...
    dispatch_request = get_dispatch_request_projection(db_session, job_run_id=job_run_id)
    payload = build_queue_enqueue_payload(dispatch_request)
    ledger = map_worker_attestation_seal_to_final_attestation_ledger(
        map_worker_completion_attestation_to_attestation_seal(
            map_worker_response_certification_to_completion_attestation(
                map_worker_response_assurance_to_response_certification(
                    map_worker_recovery_verification_to_response_assurance(
    ...
    ledger_values = [record.ledger_type for record in ledger.records]
    assert ledger_values == [
        "noop_final_attestation_ledger",
        "final_attestation_ledger_summary",
        "ledger_action_record",
        "notarization_note",
        "final_ledger_artifact",
    ]


def test_lineage_and_correlation_are_preserved_through_worker_final_attestation_ledger_normalization(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
...
def test_repeated_enqueue_calls_stay_idempotent_with_worker_final_attestation_ledger_generation(
    client,
    db_session: Session,
    monkeypatch,
) -> None:
...
def test_summaries_remain_consistent_after_worker_final_attestation_ledger_generation(
    client,
    db_session: Session,
    monkeypatch,
) -> None:
    ...
    ledger = job_run.result_summary["queue_enqueue"]["enqueue_metadata"][
