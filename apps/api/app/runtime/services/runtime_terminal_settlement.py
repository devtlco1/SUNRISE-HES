...
def bridge_runtime_protocol_reconciliation_to_terminal_settlement(
    session: Session,
    *,
    ...
    if attempt.job_run_id is None:
        raise HTTPException(
            ...
            detail="Runtime terminal settlement requires an attempt linked to a job run.",
        )
    ...
    existing = _load_runtime_terminal_settlement(attempt.execution_metadata)
    if existing is not None:
        if existing.session_identifier != payload.session_identifier:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Runtime terminal settlement is already recorded for another session.",
            )
        if existing.executor_identifier != payload.executor_identifier:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Runtime terminal settlement is already owned by another executor.",
            )
        return RuntimeTerminalSettlementBridgeResponse(
    ...
    if session_result is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal settlement requires a finalized runtime session.",
        )
    if session_result.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal settlement does not match the finalized runtime session.",
        )
    ...
    if outcome is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal settlement requires a recorded runtime execution outcome.",
        )
    if outcome.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal settlement does not match the recorded runtime execution outcome.",
        )
    ...
    if disposition is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal settlement requires a recorded runtime attempt disposition.",
        )
    if disposition.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal settlement does not match the recorded runtime attempt disposition.",
        )
    ...
    if post_processing is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal settlement requires a recorded runtime post-processing bridge.",
        )
    if post_processing.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal settlement does not match the recorded runtime post-processing bridge.",
        )
    ...
    if materialization is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal settlement requires a recorded runtime follow-up materialization.",
        )
    if materialization.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal settlement does not match the recorded runtime follow-up materialization.",
        )
    ...
    if closure is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal settlement requires a recorded runtime operational closure.",
        )
    if closure.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal settlement does not match the recorded runtime operational closure.",
        )
    ...
    if intent is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal settlement requires a recorded runtime protocol execution intent.",
        )
    if intent.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal settlement does not match the recorded runtime protocol execution intent.",
        )
    ...
    if selection is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal settlement requires a recorded runtime protocol adapter selection.",
        )
    if selection.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal settlement does not match the recorded runtime protocol adapter selection.",
        )
    ...
    if dispatch_request is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal settlement requires a recorded runtime protocol dispatch request.",
        )
    if dispatch_request.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal settlement does not match the recorded runtime protocol dispatch request.",
        )
    ...
    if invocation_result is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal settlement requires a recorded runtime protocol invocation result.",
        )
    if invocation_result.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal settlement does not match the recorded runtime protocol invocation result.",
        )
    ...
    if observation is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal settlement requires a recorded runtime protocol execution observation.",
        )
    if observation.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal settlement does not match the recorded runtime protocol execution observation.",
        )
    ...
    if interpretation is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal settlement requires a recorded runtime protocol interpretation.",
        )
    if interpretation.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal settlement does not match the recorded runtime protocol interpretation.",
        )
    ...
    if reconciliation is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal settlement requires a recorded runtime protocol reconciliation.",
        )
    ...
    if reconciliation.interpretation_record_id != interpretation.interpretation_record_id:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol reconciliation does not match the recorded runtime protocol interpretation.",
        )

    _validate_runtime_terminal_settlement_context(
        execution_metadata=attempt.execution_metadata,
        executor_identifier=payload.executor_identifier,
    )

    result = _build_runtime_terminal_settlement_result(
        attempt=attempt,
        reconciliation=reconciliation,
        payload=payload,
    )
    settlement_payload = {"runtime_terminal_settlement": result.model_dump(mode="json")}
    attempt.execution_metadata = merge_runtime_metadata(
        attempt.execution_metadata,
        settlement_payload,
    )
    command.result_summary = merge_runtime_metadata(command.result_summary, settlement_payload)
    job_run.result_summary = merge_runtime_metadata(job_run.result_summary, settlement_payload)
...
def _build_runtime_terminal_settlement_result(
    *,
    ...
    return RuntimeTerminalSettlementResult(
        status=RuntimeTerminalSettlementStatus.RECORDED,
        settlement_record_id=(
            "runtime-terminal-settlement:"
            f"{attempt.id}:{reconciliation.reconciliation_record_id}"
        ),
        session_identifier=reconciliation.session_identifier,
        reconciliation_record_id=reconciliation.reconciliation_record_id,
        interpretation_record_id=reconciliation.interpretation_record_id,
        observation_record_id=reconciliation.observation_record_id,
        invocation_result_record_id=reconciliation.invocation_result_record_id,
        dispatch_request_record_id=reconciliation.dispatch_request_record_id,
        selection_record_id=reconciliation.selection_record_id,
        intent_record_id=reconciliation.intent_record_id,
        closure_record_id=reconciliation.closure_record_id,
        materialization_record_id=reconciliation.materialization_record_id,
        post_processing_record_id=reconciliation.post_processing_record_id,
        disposition_record_id=reconciliation.disposition_record_id,
        outcome_record_id=reconciliation.outcome_record_id,
        executor_identifier=payload.executor_identifier,
        job_run_id=str(attempt.job_run_id),
        related_command_id=str(attempt.meter_command_id),
        command_attempt_id=str(attempt.id),
        terminal_outcome=reconciliation.terminal_outcome,
        downstream_state=reconciliation.downstream_state,
        settlement_key="placeholder.runtime.terminal-settlement",
        settlement_family=(
            RuntimeTerminalSettlementFamily.PLACEHOLDER_TERMINAL_RUNTIME_SETTLEMENT
        ),
        settlement_payload=RuntimeTerminalSettlementPayload(
            schema_version="placeholder-runtime-terminal-settlement.v1",
            settlement_state=(
                RuntimeTerminalSettlementState.PLACEHOLDER_TERMINAL_RUNTIME_SETTLEMENT_RECORDED
            ),
            terminal_projection_mode=reconciliation_payload.runtime_reconciliation_mode,
            adapter_family=reconciliation_payload.adapter_family,
            capability_profile=reconciliation_payload.capability_profile,
            terminal_runtime_settlement_classification=(
                RuntimeTerminalSettlementClassification.PLACEHOLDER_FINAL_RUNTIME_PROJECTION_READY
            ),
        ...
        included_follow_up_descriptor_types=reconciliation.included_follow_up_descriptor_types,
        settlement_recorded_at=datetime.now(UTC).isoformat(),
        settlement_recorded_by_executor_identifier=payload.executor_identifier,
        settlement_reason=payload.settlement_reason,
        already_recorded=False,
        summary=(
            "Runtime protocol reconciliation is converted into a durable placeholder "
            "terminal runtime settlement artifact without executing any live runtime settlement logic."
        ),
        lineage=reconciliation.lineage,
    )


def _validate_runtime_terminal_settlement_context(
    *,
    ...
    if lease is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal settlement requires a runtime lease.",
        )
    ...
    if invocation is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal settlement requires a runtime invocation gate.",
        )
    ...
    if guard is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal settlement requires a runtime execution guard.",
        )
