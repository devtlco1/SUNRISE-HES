...
from app.modules.commands.schemas import (
    ...
    RelayControlRuntimeHandoffResult,
)
from app.modules.commands.service import (
    _load_relay_control_attempt_bootstrap,
    _validate_relay_control_endpoint_assignment,
    _validate_relay_control_normalized_payload,
    _validate_relay_control_protocol_profile,
    get_meter_command,
...
RELAY_CONTROL_RUNTIME_JOB_DEFINITION_CODE = "internal-relay-control-runtime-handoff"
RELAY_CONTROL_RUNTIME_WORKER_PATH = "relay_control_runtime_handoff"


def handoff_relay_control_command_to_runtime(
    session: Session,
    *,
    ...
    if latest_attempt is not None:
        existing_handoff = _load_relay_control_runtime_handoff(latest_attempt.execution_metadata)
        existing_execution = _load_runtime_relay_control_execution(latest_attempt.execution_metadata)
        if existing_handoff is not None:
            if existing_handoff.get("handoff_identifier") != payload.handoff_identifier:
                raise HTTPException(
                    status_code=status.HTTP_409_CONFLICT,
                    detail="Relay-control runtime handoff already exists for another handoff identifier.",
                )
            if existing_handoff.get("executor_identifier") != payload.executor_identifier:
                raise HTTPException(
                    status_code=status.HTTP_409_CONFLICT,
                    detail="Relay-control runtime handoff is already owned by another executor.",
                )
            if existing_execution is not None:
                return _build_existing_relay_control_runtime_handoff_response(
                    session,
    ...
    if command.command_template.category not in {
        ...
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Selected command is not compatible with the relay-control runtime handoff slice.",
        )
    if command.current_status in {
        ...
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Relay-control command is not runtime-handoff-eligible from its current state.",
        )
    ...
    attempt = _load_bootstrapped_relay_control_attempt(session, command_id=command.id)
    bootstrap_record = _validate_bootstrapped_relay_control_attempt_for_runtime_handoff(
        attempt=attempt,
        command=command,
        normalized_payload=normalized_payload,
        endpoint_id=assignment.endpoint_id,
    )
    existing_handoff = _load_relay_control_runtime_handoff(attempt.execution_metadata)
    existing_execution = _load_runtime_relay_control_execution(attempt.execution_metadata)
    if existing_handoff is None and existing_execution is not None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Relay-control attempt already has incompatible runtime execution state without a matching handoff artifact.",
        )
    ...
    session_identifier = _get_or_finalize_runtime_execution_session(
        ...
        finalize_reason=payload.handoff_reason or "relay-control-runtime-handoff",
    )
    record_runtime_execution_outcome(
        session,
        attempt_id=attempt.id,
        payload=RuntimeExecutionOutcomeCheckpointRequest(
            ...
            outcome_reason=payload.handoff_reason or "relay-control-runtime-handoff",
            summary_message=(
                "Relay-control runtime handoff advanced the bootstrapped attempt "
                "into the bounded runtime relay-control slice."
            ),
        ),
    )
    _bridge_relay_control_runtime_chain(
        session,
        attempt_id=attempt.id,
        executor_identifier=payload.executor_identifier,
        session_identifier=session_identifier,
    ...
    relay_control_response = execute_runtime_relay_control_adapter(
        session,
        attempt_id=attempt.id,
        payload=RuntimeRelayControlExecutionRequest(
            ...
            execution_reason=payload.handoff_reason or "relay-control-runtime-handoff",
        ),
    )
    ...
    if refreshed_attempt is None or refreshed_command is None or refreshed_job_run is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Relay-control runtime handoff could not load durable runtime state.",
        )
    relay_control_execution = _load_runtime_relay_control_execution(
        refreshed_attempt.execution_metadata
    )
    if relay_control_execution is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Relay-control runtime handoff did not record a runtime relay-control execution artifact.",
        )
    handoff_record = _build_relay_control_runtime_handoff_record(
        ...
        session_identifier=session_identifier,
        relay_control_execution=relay_control_execution,
        reused_existing_handoff=existing_handoff is not None,
        reused_existing_runtime_execution=relay_control_response.result.already_recorded,
    )
    _persist_relay_control_runtime_handoff_record(
        session,
        attempt=refreshed_attempt,
    ...
    return RelayControlRuntimeHandoffResponse(
        result=RelayControlRuntimeHandoffResult(
            ...
            runtime_relay_control_execution_present=bool(
                handoff_record["runtime_relay_control_execution_present"]
            ),
            runtime_relay_control_execution_record_id=(
                str(handoff_record["runtime_relay_control_execution_record_id"])
                if handoff_record.get("runtime_relay_control_execution_record_id") is not None
                else None
            ),
...
def _load_bootstrapped_relay_control_attempt(
    session: Session,
    *,
    command_id: uuid.UUID,
    ...
    if attempt is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Relay-control command does not have a bootstrapped attempt for runtime handoff.",
        )
    return attempt
...
def _build_existing_relay_control_runtime_handoff_response(
    session: Session,
    *,
    ...
    if attempt.job_run_id is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Relay-control runtime handoff artifact references a missing job run.",
        )
    job_run = session.get(JobRun, attempt.job_run_id)
    if job_run is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Relay-control runtime handoff artifact references an unknown job run.",
        )
    relay_operation = RelayControlCommandOperation(
        str(handoff_record["relay_control_operation"])
    )
    return RelayControlRuntimeHandoffResponse(
        result=RelayControlRuntimeHandoffResult(
            handoff_status=str(handoff_record["handoff_status"]),
...
def _validate_bootstrapped_relay_control_attempt_for_runtime_handoff(
    *,
    attempt: CommandExecutionAttempt,
    command: MeterCommand,
    ...
    bootstrap_record = _load_relay_control_attempt_bootstrap(attempt.execution_metadata)
    if bootstrap_record is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Relay-control attempt is missing the bootstrap artifact required for runtime handoff.",
        )
    if attempt.status != CommandExecutionAttemptStatus.STARTED or attempt.ended_at is not None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Relay-control attempt is not runtime-handoff-eligible from its current state.",
        )
    if not isinstance(attempt.request_snapshot, dict):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Relay-control attempt is missing a request snapshot for runtime handoff.",
        )
    if attempt.request_snapshot != normalized_payload:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Relay-control attempt request snapshot is incompatible with the normalized payload for runtime handoff.",
        )
    if attempt.endpoint_id != endpoint_id:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Relay-control attempt endpoint continuity is incompatible with runtime handoff.",
        )
    if str(bootstrap_record.get("command_id")) != str(command.id):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Relay-control attempt bootstrap artifact is incompatible with the selected command.",
        )
    if str(bootstrap_record.get("relay_control_operation")) != str(
        normalized_payload["relay_control_operation"]
    ):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Relay-control attempt bootstrap artifact is incompatible with the bounded relay-control runtime slice.",
        )
...
def _get_or_create_runtime_job_definition(session: Session) -> JobDefinition:
    job_definition = session.scalar(
        select(JobDefinition).where(
            func.lower(JobDefinition.code) == RELAY_CONTROL_RUNTIME_JOB_DEFINITION_CODE
        )
    )
    if job_definition is not None:
        if job_definition.category != JobCategory.COMMAND:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Existing internal relay-control runtime job definition is incompatible.",
            )
        return job_definition
    job_definition = JobDefinition(
        code=RELAY_CONTROL_RUNTIME_JOB_DEFINITION_CODE,
        name="Internal Relay Control Runtime Handoff",
        ...
        max_retries=0,
        is_active=True,
        notes="Internal bounded job definition for relay-control command-to-runtime handoff.",
    )
    ...
    *,
    ...
    if attempt.job_run_id is not None:
        ...
        if job_run is None:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Relay-control attempt references a missing job run for runtime handoff.",
            )
        if job_run.related_command_id != command.id:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Relay-control attempt references an incompatible job run for runtime handoff.",
            )
        if job_run.target_endpoint_id != endpoint_id:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Relay-control runtime handoff detected inconsistent endpoint continuity on the existing job run.",
            )
        return job_run
...
def _persist_runtime_execution_handoff(
    ...
    *,
    ...
    result = RuntimeExecutionHandoffResult(
        ...
        handoff_record_id=f"relay-control-runtime-handoff:{attempt.id}",
        stream_name="relay-control-runtime-handoff",
        consumer_group="relay-control-runtime-handoff",
        consumer_name=f"relay-control-runtime:{executor_identifier}",
        worker_identifier=executor_identifier,
        ...
        summary=(
            "Relay-control command runtime handoff linked the bootstrapped attempt "
        ...
        lineage=RuntimeExecutionHandoffLineage(
            ...
            queue_message_id=f"relay-control-runtime-handoff-message:{attempt.id}",
            claim_token=f"relay-control-runtime-handoff-claim:{handoff_identifier}",
            source_identifiers={
                "job_run_id": str(job_run.id),
            ...
            dispatch_metadata={
                "synthetic": True,
                "source": "relay_control_command_runtime_handoff",
                "handoff_identifier": handoff_identifier,
            },
            intended_worker_path=RELAY_CONTROL_RUNTIME_WORKER_PATH,
        ),
    )
...
def _ensure_runtime_execution_guard(
    ...
    *,
    ...
    if attempt is None or command is None or job_run is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Relay-control runtime handoff could not load durable runtime guard state.",
        )
...
def _bridge_relay_control_runtime_chain(
    session: Session,
    *,
    attempt_id: uuid.UUID,
    executor_identifier: str,
    ...
    reason = handoff_reason or "relay-control-runtime-handoff"
    bridge_runtime_execution_outcome_to_attempt_disposition(
        session,
        attempt_id=attempt_id,
...
def _build_relay_control_runtime_handoff_record(
    *,
    attempt: CommandExecutionAttempt,
    job_run: JobRun,
    ...
    relay_operation: RelayControlCommandOperation,
    session_identifier: str,
    relay_control_execution: RuntimeRelayControlExecutionResult,
    ...
    return {
        ...
        "runtime_relay_control_execution_present": True,
        "runtime_relay_control_execution_record_id": (
            relay_control_execution.relay_control_execution_record_id
        ),
        ...
        "handed_off_at": datetime.now(UTC).isoformat(),
        "trace_references": {
            "dispatch_envelope_record_id": relay_control_execution.dispatch_envelope_record_id,
            "correlation_id": relay_control_execution.correlation_id,
            "request_id": relay_control_execution.request_id,
        },
    }


def _persist_relay_control_runtime_handoff_record(
    session: Session,
    *,
    attempt: CommandExecutionAttempt,
    command: MeterCommand,
    ...
    payload = {"relay_control_runtime_handoff": handoff_record}
...
def _load_relay_control_runtime_handoff(
    execution_metadata: dict[str, object] | None,
) -> dict[str, object] | None:
    if not isinstance(execution_metadata, dict):
        return None
    payload = execution_metadata.get("relay_control_runtime_handoff")
