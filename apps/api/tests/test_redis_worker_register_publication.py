...
def _build_register_publication_for_retry_job(client, db_session: Session):
    ...
    register_publication = map_worker_register_finalization_to_register_publication(
        map_worker_register_closeout_to_register_finalization(
            map_worker_register_reconciliation_to_register_closeout(
                map_worker_retention_register_to_register_reconciliation(
                    map_worker_closure_archive_to_retention_register(
    ...
    return job_run_id, dispatch_request, register_publication


def test_register_finalization_maps_into_stable_register_publication_records(
    client, db_session: Session
) -> None:
    _, _, publication = _build_register_publication_for_retry_job(client, db_session)

    assert publication.records[0].publication_type == "noop_register_publication"
    assert publication.records[1].publication_type == "register_publication_summary"
    assert publication.records[2].publication_type == "publication_action_record"
    assert publication.records[3].publication_type == "publication_note"
    assert publication.records[4].publication_type == "workflow_completion_artifact"
    assert publication.total_records == 5
    assert publication.terminal_records == 4
    assert publication.publication_ready is True


def test_register_publication_artifacts_are_exposed_correctly(client, db_session: Session) -> None:
    _, _, publication = _build_register_publication_for_retry_job(client, db_session)

    publication_values = [record.publication_type for record in publication.records]
    assert publication_values == [
        "noop_register_publication",
        "register_publication_summary",
        "publication_action_record",
        "publication_note",
        "workflow_completion_artifact",
    ]


def test_lineage_and_correlation_are_preserved_through_worker_register_publication(
    client, db_session: Session
) -> None:
    job_run_id, dispatch_request, publication = _build_register_publication_for_retry_job(
        client, db_session
    )

    assert publication.metadata["source_identifiers"]["job_run_id"] == job_run_id
    assert publication.metadata["source_identifiers"]["command_id"] is not None
    assert (
        publication.metadata["correlation_lineage"]["derived_correlation_id"]
...
def test_repeated_enqueue_calls_stay_idempotent_with_worker_register_publication_generation(
...
def test_summaries_remain_consistent_after_worker_register_publication_generation(
    client, db_session: Session, monkeypatch
) -> None:
    monkeypatch.setattr(settings, "queue_backend", "redis_placeholder")
    token = _login_as_super_admin(client, db_session)
    ...
    publication = job_run.result_summary["queue_enqueue"]["enqueue_metadata"][
