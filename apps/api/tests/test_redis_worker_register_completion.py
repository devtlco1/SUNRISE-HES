...
def _build_register_completion_for_retry_job(client, db_session: Session):
    ...
    register_completion = map_worker_register_publication_to_register_completion(
        map_worker_register_finalization_to_register_publication(
            map_worker_register_closeout_to_register_finalization(
                map_worker_register_reconciliation_to_register_closeout(
                    map_worker_retention_register_to_register_reconciliation(
    ...
    return job_run_id, dispatch_request, register_completion


def test_register_publication_maps_into_stable_register_completion_records(
    client, db_session: Session
) -> None:
    _, _, completion = _build_register_completion_for_retry_job(client, db_session)

    assert completion.records[0].completion_type == "noop_register_completion"
    assert completion.records[1].completion_type == "register_completion_summary"
    assert completion.records[2].completion_type == "completion_action_record"
    assert completion.records[3].completion_type == "register_completion_note"
    assert completion.records[4].completion_type == "post_publication_completion_artifact"
    assert completion.total_records == 5
    assert completion.terminal_records == 4
    assert completion.completion_ready is True


def test_register_completion_artifacts_are_exposed_correctly(client, db_session: Session) -> None:
    _, _, completion = _build_register_completion_for_retry_job(client, db_session)

    completion_values = [record.completion_type for record in completion.records]
    assert completion_values == [
        "noop_register_completion",
        "register_completion_summary",
        "completion_action_record",
        "register_completion_note",
        "post_publication_completion_artifact",
    ]


def test_lineage_and_correlation_are_preserved_through_worker_register_completion(
    client, db_session: Session
) -> None:
    job_run_id, dispatch_request, completion = _build_register_completion_for_retry_job(
        client, db_session
    )

    assert completion.metadata["source_identifiers"]["job_run_id"] == job_run_id
    assert completion.metadata["source_identifiers"]["command_id"] is not None
    assert (
        completion.metadata["correlation_lineage"]["derived_correlation_id"]
...
def test_repeated_enqueue_calls_stay_idempotent_with_worker_register_completion_generation(
...
def test_summaries_remain_consistent_after_worker_register_completion_generation(
    client, db_session: Session, monkeypatch
) -> None:
    monkeypatch.setattr(settings, "queue_backend", "redis_placeholder")
    token = _login_as_super_admin(client, db_session)
    ...
    completion = job_run.result_summary["queue_enqueue"]["enqueue_metadata"][
