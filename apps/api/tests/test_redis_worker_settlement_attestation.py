...
def _build_settlement_attestation_for_retry_job(client, db_session: Session):
    ...
    settlement_attestation = map_worker_register_settlement_to_settlement_attestation(
        map_worker_register_completion_to_register_settlement(
            map_worker_register_publication_to_register_completion(
                map_worker_register_finalization_to_register_publication(
                    map_worker_register_closeout_to_register_finalization(
    ...
    return job_run_id, dispatch_request, settlement_attestation


def test_register_settlement_maps_into_stable_settlement_attestation_records(
    client, db_session: Session
) -> None:
    _, _, settlement_attestation = _build_settlement_attestation_for_retry_job(client, db_session)

    assert settlement_attestation.records[0].attestation_type == "noop_settlement_attestation"
    assert settlement_attestation.records[1].attestation_type == "settlement_attestation_summary"
    assert settlement_attestation.records[2].attestation_type == "attestation_action_record"
    assert settlement_attestation.records[3].attestation_type == "settlement_attestation_note"
    assert settlement_attestation.records[4].attestation_type == "post_settlement_proof_artifact"
    assert settlement_attestation.total_records == 5
    assert settlement_attestation.terminal_records == 4
    assert settlement_attestation.attestation_ready is True


def test_settlement_attestation_artifacts_are_exposed_correctly(
    client, db_session: Session
) -> None:
    _, _, settlement_attestation = _build_settlement_attestation_for_retry_job(client, db_session)

    attestation_values = [record.attestation_type for record in settlement_attestation.records]
    assert attestation_values == [
        "noop_settlement_attestation",
        "settlement_attestation_summary",
        "attestation_action_record",
        "settlement_attestation_note",
        "post_settlement_proof_artifact",
    ]


def test_lineage_and_correlation_are_preserved_through_worker_settlement_attestation(
    client, db_session: Session
) -> None:
    (
        job_run_id,
    ...
    ) = _build_settlement_attestation_for_retry_job(client, db_session)

    assert settlement_attestation.metadata["source_identifiers"]["job_run_id"] == job_run_id
    assert settlement_attestation.metadata["source_identifiers"]["command_id"] is not None
    assert (
        settlement_attestation.metadata["correlation_lineage"]["derived_correlation_id"]
...
def test_repeated_enqueue_calls_stay_idempotent_with_worker_settlement_attestation_generation(
    ...
    assert (
        first.json()["enqueue_result"]["adapter_receipt_id"]
        == second.json()["enqueue_result"]["adapter_receipt_id"]
    )


def test_summaries_remain_consistent_after_worker_settlement_attestation_generation(
    client, db_session: Session, monkeypatch
) -> None:
    monkeypatch.setattr(settings, "queue_backend", "redis_placeholder")
    token = _login_as_super_admin(client, db_session)
    ...
    settlement_attestation = job_run.result_summary["queue_enqueue"]["enqueue_metadata"][
        "redis_worker_settlement_attestation"
    ]
    assert settlement_attestation["records"][0]["attestation_type"] == "noop_settlement_attestation"
    assert (
        settlement_attestation["records"][1]["attestation_type"] == "settlement_attestation_summary"
    )
    assert settlement_attestation["records"][3]["attestation_type"] == "settlement_attestation_note"
    assert settlement_attestation["total_records"] == 5
    assert settlement_attestation["attestation_ready"] is True
