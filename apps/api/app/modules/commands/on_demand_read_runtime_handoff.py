...
from app.modules.commands.schemas import (
    ...
    OnDemandReadRuntimeHandoffResult,
)
from app.modules.commands.service import (
    ON_DEMAND_READ_RUNTIME_OPERATION,
    ON_DEMAND_READ_SNAPSHOT_TYPE,
    _load_on_demand_read_attempt_bootstrap,
    _validate_on_demand_read_endpoint_assignment,
    _validate_on_demand_read_normalized_payload,
    _validate_on_demand_read_protocol_profile,
    get_meter_command,
...
ON_DEMAND_READ_RUNTIME_JOB_DEFINITION_CODE = "internal-on-demand-read-runtime-handoff"
ON_DEMAND_READ_RUNTIME_WORKER_PATH = "on_demand_read_runtime_handoff"


def handoff_on_demand_read_command_to_runtime(
    session: Session,
    *,
    ...
    if latest_attempt is not None:
        existing_handoff = _load_on_demand_read_runtime_handoff(latest_attempt.execution_metadata)
        existing_execution = _load_runtime_on_demand_read_execution(
        ...
        if existing_handoff is not None:
            if existing_handoff.get("handoff_identifier") != payload.handoff_identifier:
                raise HTTPException(
                    ...
                    detail="On-demand-read runtime handoff already exists for another handoff identifier.",
                )
            if existing_handoff.get("executor_identifier") != payload.executor_identifier:
                raise HTTPException(
                    status_code=status.HTTP_409_CONFLICT,
                    detail="On-demand-read runtime handoff is already owned by another executor.",
                )
            if existing_execution is not None:
                return _build_existing_on_demand_read_runtime_handoff_response(
                    session,
    ...
    if command.command_template.category != CommandCategory.ON_DEMAND_READ:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Selected command is not compatible with the on-demand-read runtime handoff slice.",
        )
    if command.current_status in {
        ...
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="On-demand-read command is not runtime-handoff-eligible from its current state.",
        )
    ...
    attempt = _load_bootstrapped_on_demand_read_attempt(session, command_id=command.id)
    bootstrap_record = _validate_bootstrapped_on_demand_read_attempt_for_runtime_handoff(
        attempt=attempt,
        command=command,
        normalized_payload=normalized_payload,
        endpoint_id=assignment.endpoint_id,
    )
    existing_handoff = _load_on_demand_read_runtime_handoff(attempt.execution_metadata)
    existing_execution = _load_runtime_on_demand_read_execution(attempt.execution_metadata)
    if existing_handoff is None and existing_execution is not None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="On-demand-read attempt already has incompatible runtime execution state without a matching handoff artifact.",
        )
    ...
    session_identifier = _get_or_finalize_runtime_execution_session(
        ...
        finalize_reason=payload.handoff_reason or "on-demand-read-runtime-handoff",
    )
    record_runtime_execution_outcome(
        session,
        attempt_id=attempt.id,
        payload=RuntimeExecutionOutcomeCheckpointRequest(
            ...
            outcome_reason=payload.handoff_reason or "on-demand-read-runtime-handoff",
            summary_message=(
                "On-demand-read runtime handoff advanced the bootstrapped attempt "
                "into the bounded runtime on-demand-read slice."
            ),
        ),
    )
    _bridge_on_demand_read_runtime_chain(
        session,
        attempt_id=attempt.id,
        executor_identifier=payload.executor_identifier,
        session_identifier=session_identifier,
    ...
    on_demand_read_response = execute_runtime_on_demand_read_adapter(
        session,
        attempt_id=attempt.id,
        payload=RuntimeOnDemandReadExecutionRequest(
            ...
            execution_reason=payload.handoff_reason or "on-demand-read-runtime-handoff",
        ),
    )
    ...
    if refreshed_attempt is None or refreshed_command is None or refreshed_job_run is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="On-demand-read runtime handoff could not load durable runtime state.",
        )
    on_demand_read_execution = _load_runtime_on_demand_read_execution(
        refreshed_attempt.execution_metadata
    )
    if on_demand_read_execution is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="On-demand-read runtime handoff did not record a runtime on-demand-read execution artifact.",
        )
    handoff_record = _build_on_demand_read_runtime_handoff_record(
        ...
        session_identifier=session_identifier,
        on_demand_read_execution=on_demand_read_execution,
        reused_existing_handoff=existing_handoff is not None,
        reused_existing_runtime_execution=on_demand_read_response.result.already_recorded,
    )
    _persist_on_demand_read_runtime_handoff_record(
        session,
        attempt=refreshed_attempt,
    ...
    return OnDemandReadRuntimeHandoffResponse(
        result=OnDemandReadRuntimeHandoffResult(
            ...
            runtime_on_demand_read_execution_present=bool(
                handoff_record["runtime_on_demand_read_execution_present"]
            ),
            runtime_on_demand_read_execution_record_id=(
                str(handoff_record["runtime_on_demand_read_execution_record_id"])
                if handoff_record.get("runtime_on_demand_read_execution_record_id") is not None
                else None
            ),
...
def _load_bootstrapped_on_demand_read_attempt(
    session: Session,
    *,
    command_id: uuid.UUID,
    ...
    if attempt is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="On-demand-read command does not have a bootstrapped attempt for runtime handoff.",
        )
    return attempt
...
def _build_existing_on_demand_read_runtime_handoff_response(
    session: Session,
    *,
    ...
    if attempt.job_run_id is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="On-demand-read runtime handoff artifact references a missing job run.",
        )
    job_run = session.get(JobRun, attempt.job_run_id)
    if job_run is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="On-demand-read runtime handoff artifact references an unknown job run.",
        )
    return OnDemandReadRuntimeHandoffResponse(
        result=OnDemandReadRuntimeHandoffResult(
            handoff_status=str(handoff_record["handoff_status"]),
            ...
            bootstrap_identifier=str(handoff_record["bootstrap_identifier"]),
            on_demand_read_operation=OnDemandReadCommandOperation(
                str(handoff_record["on_demand_read_operation"])
            ),
            snapshot_type=SnapshotType(str(handoff_record["snapshot_type"])),
            handed_off_at=datetime.fromisoformat(str(handoff_record["handed_off_at"])),
            session_identifier=str(handoff_record["session_identifier"]),
            runtime_on_demand_read_execution_present=bool(
                handoff_record["runtime_on_demand_read_execution_present"]
...
def _validate_bootstrapped_on_demand_read_attempt_for_runtime_handoff(
    *,
    attempt: CommandExecutionAttempt,
    command: MeterCommand,
    ...
    bootstrap_record = _load_on_demand_read_attempt_bootstrap(attempt.execution_metadata)
    if bootstrap_record is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="On-demand-read attempt is missing the bootstrap artifact required for runtime handoff.",
        )
    if attempt.status != CommandExecutionAttemptStatus.STARTED or attempt.ended_at is not None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="On-demand-read attempt is not runtime-handoff-eligible from its current state.",
        )
    if not isinstance(attempt.request_snapshot, dict):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="On-demand-read attempt is missing a request snapshot for runtime handoff.",
        )
    if attempt.request_snapshot != normalized_payload:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="On-demand-read attempt request snapshot is incompatible with the normalized payload for runtime handoff.",
        )
    if attempt.endpoint_id != endpoint_id:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="On-demand-read attempt endpoint continuity is incompatible with runtime handoff.",
        )
    if str(bootstrap_record.get("command_id")) != str(command.id):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="On-demand-read attempt bootstrap artifact is incompatible with the selected command.",
        )
    if str(bootstrap_record.get("on_demand_read_operation")) != ON_DEMAND_READ_RUNTIME_OPERATION:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="On-demand-read attempt bootstrap artifact is incompatible with the bounded on-demand-read runtime slice.",
        )
    if str(bootstrap_record.get("snapshot_type")) != ON_DEMAND_READ_SNAPSHOT_TYPE.value:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="On-demand-read attempt bootstrap artifact is incompatible with the bounded on-demand-read runtime slice.",
        )
...
def _get_or_create_runtime_job_definition(session: Session) -> JobDefinition:
    job_definition = session.scalar(
        select(JobDefinition).where(
            func.lower(JobDefinition.code) == ON_DEMAND_READ_RUNTIME_JOB_DEFINITION_CODE
        )
    )
    if job_definition is not None:
        if job_definition.category != JobCategory.COMMAND:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Existing internal on-demand-read runtime job definition is incompatible.",
            )
        return job_definition
    job_definition = JobDefinition(
        code=ON_DEMAND_READ_RUNTIME_JOB_DEFINITION_CODE,
        name="Internal On Demand Read Runtime Handoff",
        ...
        max_retries=0,
        is_active=True,
        notes="Internal bounded job definition for on-demand-read command-to-runtime handoff.",
    )
    ...
    *,
    ...
    if attempt.job_run_id is not None:
        ...
        if job_run is None:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="On-demand-read attempt references a missing job run for runtime handoff.",
            )
        if job_run.related_command_id != command.id:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="On-demand-read attempt references an incompatible job run for runtime handoff.",
            )
        if job_run.target_endpoint_id != endpoint_id:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="On-demand-read runtime handoff detected inconsistent endpoint continuity on the existing job run.",
            )
        return job_run
...
def _persist_runtime_execution_handoff(
    ...
    *,
    ...
    result = RuntimeExecutionHandoffResult(
        ...
        handoff_record_id=f"on-demand-read-runtime-handoff:{attempt.id}",
        stream_name="on-demand-read-runtime-handoff",
        consumer_group="on-demand-read-runtime-handoff",
        consumer_name=f"on-demand-read-runtime:{executor_identifier}",
        worker_identifier=executor_identifier,
        ...
        summary=(
            "On-demand-read command runtime handoff linked the bootstrapped attempt "
        ...
        lineage=RuntimeExecutionHandoffLineage(
            ...
            queue_message_id=f"on-demand-read-runtime-handoff-message:{attempt.id}",
            claim_token=f"on-demand-read-runtime-handoff-claim:{handoff_identifier}",
            source_identifiers={
                "job_run_id": str(job_run.id),
            ...
            dispatch_metadata={
                "synthetic": True,
                "source": "on_demand_read_command_runtime_handoff",
                "handoff_identifier": handoff_identifier,
            },
            intended_worker_path=ON_DEMAND_READ_RUNTIME_WORKER_PATH,
        ),
    )
...
def _ensure_runtime_execution_guard(
    ...
    *,
    ...
    if attempt is None or command is None or job_run is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="On-demand-read runtime handoff could not load durable runtime guard state.",
        )
...
def _bridge_on_demand_read_runtime_chain(
    session: Session,
    *,
    attempt_id: uuid.UUID,
    executor_identifier: str,
    ...
    reason = handoff_reason or "on-demand-read-runtime-handoff"
    bridge_runtime_execution_outcome_to_attempt_disposition(
        session,
        attempt_id=attempt_id,
    ...
    bridge_runtime_closure_attestation_to_publication_contract(
        ...
        payload=RuntimePublicationContractBridgeRequest(
            executor_identifier=executor_identifier,
            session_identifier=session_identifier,
            publication_contract_reason=reason,
        ),
    )
    bridge_runtime_publication_contract_to_externalization_envelope(
        session,
    ...
    bridge_runtime_externalization_envelope_to_delivery_contract(
        ...
        payload=RuntimeDeliveryContractBridgeRequest(
            executor_identifier=executor_identifier,
            session_identifier=session_identifier,
            delivery_contract_reason=reason,
        ),
    )
    bridge_runtime_delivery_contract_to_dispatch_envelope(
        session,
        ...
        payload=RuntimeDispatchEnvelopeBridgeRequest(
            executor_identifier=executor_identifier,
            session_identifier=session_identifier,
            dispatch_envelope_reason=reason,
        ),
    )


def _build_on_demand_read_runtime_handoff_record(
    *,
    attempt: CommandExecutionAttempt,
    job_run: JobRun,
    ...
    session_identifier: str,
    on_demand_read_execution: RuntimeOnDemandReadExecutionResult,
    ...
    return {
        ...
        "runtime_on_demand_read_execution_present": True,
        "runtime_on_demand_read_execution_record_id": (
            on_demand_read_execution.on_demand_read_execution_record_id
        ),
        ...
        "handed_off_at": datetime.now(UTC).isoformat(),
        "trace_references": {
            "dispatch_envelope_record_id": on_demand_read_execution.dispatch_envelope_record_id,
            "correlation_id": on_demand_read_execution.correlation_id,
            "request_id": on_demand_read_execution.request_id,
        },
    }


def _persist_on_demand_read_runtime_handoff_record(
    session: Session,
    *,
    attempt: CommandExecutionAttempt,
    command: MeterCommand,
    ...
    payload = {"on_demand_read_runtime_handoff": handoff_record}
