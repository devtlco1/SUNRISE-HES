...
def _build_register_finalization_for_retry_job(client, db_session: Session):
    ...
    register_finalization = map_worker_register_closeout_to_register_finalization(
        map_worker_register_reconciliation_to_register_closeout(
            map_worker_retention_register_to_register_reconciliation(
                map_worker_closure_archive_to_retention_register(
                    map_worker_ledger_closure_to_closure_archive(
    ...
    return job_run_id, dispatch_request, register_finalization


def test_register_closeout_maps_into_stable_register_finalization_records(
    client, db_session: Session
) -> None:
    _, _, finalization = _build_register_finalization_for_retry_job(client, db_session)

    assert finalization.records[0].finalization_type == "noop_register_finalization"
    assert finalization.records[1].finalization_type == "register_finalization_summary"
    assert finalization.records[2].finalization_type == "finalization_action_record"
    assert finalization.records[3].finalization_type == "register_finalization_note"
    assert finalization.records[4].finalization_type == "final_register_workflow_artifact"
    assert finalization.total_records == 5
    assert finalization.terminal_records == 4
    assert finalization.finalization_ready is True


def test_register_finalization_artifacts_are_exposed_correctly(client, db_session: Session) -> None:
    _, _, finalization = _build_register_finalization_for_retry_job(client, db_session)

    finalization_values = [record.finalization_type for record in finalization.records]
    assert finalization_values == [
        "noop_register_finalization",
        "register_finalization_summary",
        "finalization_action_record",
        "register_finalization_note",
        "final_register_workflow_artifact",
    ]


def test_lineage_and_correlation_are_preserved_through_worker_register_finalization(
    client, db_session: Session
) -> None:
    job_run_id, dispatch_request, finalization = _build_register_finalization_for_retry_job(
        client, db_session
    )

    assert finalization.metadata["source_identifiers"]["job_run_id"] == job_run_id
    assert finalization.metadata["source_identifiers"]["command_id"] is not None
    assert (
        finalization.metadata["correlation_lineage"]["derived_correlation_id"]
...
def test_repeated_enqueue_calls_stay_idempotent_with_worker_register_finalization_generation(
...
def test_summaries_remain_consistent_after_worker_register_finalization_generation(
    client, db_session: Session, monkeypatch
) -> None:
    monkeypatch.setattr(settings, "queue_backend", "redis_placeholder")
    token = _login_as_super_admin(client, db_session)
    ...
    finalization = job_run.result_summary["queue_enqueue"]["enqueue_metadata"][
