...
def bridge_runtime_terminal_settlement_to_closure_attestation(
    session: Session,
    *,
    ...
    if attempt.job_run_id is None:
        raise HTTPException(
            ...
            detail="Runtime closure attestation requires an attempt linked to a job run.",
        )
    ...
    existing = _load_runtime_closure_attestation(attempt.execution_metadata)
    if existing is not None:
        if existing.session_identifier != payload.session_identifier:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Runtime closure attestation is already recorded for another session.",
            )
        if existing.executor_identifier != payload.executor_identifier:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Runtime closure attestation is already owned by another executor.",
            )
        return RuntimeClosureAttestationBridgeResponse(
    ...
    if session_result is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime closure attestation requires a finalized runtime session.",
        )
    if session_result.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime closure attestation does not match the finalized runtime session.",
        )
    ...
    if outcome is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime closure attestation requires a recorded runtime execution outcome.",
        )
    if outcome.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime closure attestation does not match the recorded runtime execution outcome.",
        )
    ...
    if disposition is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime closure attestation requires a recorded runtime attempt disposition.",
        )
    if disposition.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime closure attestation does not match the recorded runtime attempt disposition.",
        )
    ...
    if post_processing is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime closure attestation requires a recorded runtime post-processing bridge.",
        )
    if post_processing.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime closure attestation does not match the recorded runtime post-processing bridge.",
        )
    ...
    if materialization is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime closure attestation requires a recorded runtime follow-up materialization.",
        )
    if materialization.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime closure attestation does not match the recorded runtime follow-up materialization.",
        )
    ...
    if closure is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime closure attestation requires a recorded runtime operational closure.",
        )
    if closure.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime closure attestation does not match the recorded runtime operational closure.",
        )
    ...
    if intent is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime closure attestation requires a recorded runtime protocol execution intent.",
        )
    if intent.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime closure attestation does not match the recorded runtime protocol execution intent.",
        )
    ...
    if selection is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime closure attestation requires a recorded runtime protocol adapter selection.",
        )
    if selection.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime closure attestation does not match the recorded runtime protocol adapter selection.",
        )
    ...
    if dispatch_request is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime closure attestation requires a recorded runtime protocol dispatch request.",
        )
    if dispatch_request.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime closure attestation does not match the recorded runtime protocol dispatch request.",
        )
    ...
    if invocation_result is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime closure attestation requires a recorded runtime protocol invocation result.",
        )
    if invocation_result.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime closure attestation does not match the recorded runtime protocol invocation result.",
        )
    ...
    if observation is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime closure attestation requires a recorded runtime protocol execution observation.",
        )
    if observation.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime closure attestation does not match the recorded runtime protocol execution observation.",
        )
    ...
    if interpretation is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime closure attestation requires a recorded runtime protocol interpretation.",
        )
    if interpretation.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime closure attestation does not match the recorded runtime protocol interpretation.",
        )
    ...
    if reconciliation is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime closure attestation requires a recorded runtime protocol reconciliation.",
        )
    if reconciliation.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime closure attestation does not match the recorded runtime protocol reconciliation.",
        )
    ...
    if settlement is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime closure attestation requires a recorded runtime terminal settlement.",
        )
    ...
    if settlement.reconciliation_record_id != reconciliation.reconciliation_record_id:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal settlement does not match the recorded runtime protocol reconciliation.",
        )

    _validate_runtime_closure_attestation_context(
        execution_metadata=attempt.execution_metadata,
        executor_identifier=payload.executor_identifier,
    )

    result = _build_runtime_closure_attestation_result(
        attempt=attempt,
        settlement=settlement,
        payload=payload,
    )
    attestation_payload = {"runtime_closure_attestation": result.model_dump(mode="json")}
    ...
    job_run.result_summary = merge_runtime_metadata(job_run.result_summary, attestation_payload)
...
def _build_runtime_closure_attestation_result(
    *,
    attempt: CommandExecutionAttempt,
    settlement: RuntimeTerminalSettlementResult,
    payload: RuntimeClosureAttestationBridgeRequest,
) -> RuntimeClosureAttestationResult:
    settlement_payload = settlement.settlement_payload
    return RuntimeClosureAttestationResult(
        status=RuntimeClosureAttestationStatus.RECORDED,
        attestation_record_id=(
            "runtime-closure-attestation:"
            f"{attempt.id}:{settlement.settlement_record_id}"
        ),
        session_identifier=settlement.session_identifier,
        settlement_record_id=settlement.settlement_record_id,
        reconciliation_record_id=settlement.reconciliation_record_id,
        interpretation_record_id=settlement.interpretation_record_id,
        observation_record_id=settlement.observation_record_id,
        invocation_result_record_id=settlement.invocation_result_record_id,
        dispatch_request_record_id=settlement.dispatch_request_record_id,
        selection_record_id=settlement.selection_record_id,
        intent_record_id=settlement.intent_record_id,
        closure_record_id=settlement.closure_record_id,
        materialization_record_id=settlement.materialization_record_id,
        post_processing_record_id=settlement.post_processing_record_id,
        disposition_record_id=settlement.disposition_record_id,
        outcome_record_id=settlement.outcome_record_id,
        executor_identifier=payload.executor_identifier,
        ...
        downstream_state=settlement.downstream_state,
        attestation_key="placeholder.runtime.closure-attestation",
        attestation_family=(
            RuntimeClosureAttestationFamily.PLACEHOLDER_RUNTIME_CLOSURE_ATTESTATION
        ),
        attestation_payload=RuntimeClosureAttestationPayload(
            schema_version="placeholder-runtime-closure-attestation.v1",
            attestation_state=RuntimeClosureAttestationState.PLACEHOLDER_RUNTIME_CLOSURE_ATTESTED,
            finalization_projection_mode=settlement_payload.terminal_projection_mode,
            adapter_family=settlement_payload.adapter_family,
            capability_profile=settlement_payload.capability_profile,
            closure_attestation_classification=(
                RuntimeClosureAttestationClassification.PLACEHOLDER_EXTERNALLY_CONSUMABLE_FINALIZATION_READY
            ),
        ...
        included_follow_up_descriptor_types=settlement.included_follow_up_descriptor_types,
        attestation_recorded_at=datetime.now(UTC).isoformat(),
        attestation_recorded_by_executor_identifier=payload.executor_identifier,
        attestation_reason=payload.attestation_reason,
        already_recorded=False,
        summary=(
            "Runtime terminal settlement is converted into a durable placeholder "
            "runtime closure attestation artifact without executing any live finalization projection logic."
        ),
        lineage=settlement.lineage,
    )


def _validate_runtime_closure_attestation_context(
    *,
    ...
    if lease is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime closure attestation requires a runtime lease.",
        )
    ...
    if invocation is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime closure attestation requires a runtime invocation gate.",
        )
    ...
    if guard is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime closure attestation requires a runtime execution guard.",
        )
