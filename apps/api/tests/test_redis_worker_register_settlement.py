...
def _build_register_settlement_for_retry_job(client, db_session: Session):
    ...
    register_settlement = map_worker_register_completion_to_register_settlement(
        map_worker_register_publication_to_register_completion(
            map_worker_register_finalization_to_register_publication(
                map_worker_register_closeout_to_register_finalization(
                    map_worker_register_reconciliation_to_register_closeout(
    ...
    return job_run_id, dispatch_request, register_settlement


def test_register_completion_maps_into_stable_register_settlement_records(
    client, db_session: Session
) -> None:
    _, _, settlement = _build_register_settlement_for_retry_job(client, db_session)

    assert settlement.records[0].settlement_type == "noop_register_settlement"
    assert settlement.records[1].settlement_type == "register_settlement_summary"
    assert settlement.records[2].settlement_type == "settlement_action_record"
    assert settlement.records[3].settlement_type == "register_settlement_note"
    assert settlement.records[4].settlement_type == "post_completion_settlement_artifact"
    assert settlement.total_records == 5
    assert settlement.terminal_records == 4
    assert settlement.settlement_ready is True


def test_register_settlement_artifacts_are_exposed_correctly(client, db_session: Session) -> None:
    _, _, settlement = _build_register_settlement_for_retry_job(client, db_session)

    settlement_values = [record.settlement_type for record in settlement.records]
    assert settlement_values == [
        "noop_register_settlement",
        "register_settlement_summary",
        "settlement_action_record",
        "register_settlement_note",
        "post_completion_settlement_artifact",
    ]


def test_lineage_and_correlation_are_preserved_through_worker_register_settlement(
    client, db_session: Session
) -> None:
    job_run_id, dispatch_request, settlement = _build_register_settlement_for_retry_job(
        client, db_session
    )

    assert settlement.metadata["source_identifiers"]["job_run_id"] == job_run_id
    assert settlement.metadata["source_identifiers"]["command_id"] is not None
    assert (
        settlement.metadata["correlation_lineage"]["derived_correlation_id"]
...
def test_repeated_enqueue_calls_stay_idempotent_with_worker_register_settlement_generation(
...
def test_summaries_remain_consistent_after_worker_register_settlement_generation(
    client, db_session: Session, monkeypatch
) -> None:
    monkeypatch.setattr(settings, "queue_backend", "redis_placeholder")
    token = _login_as_super_admin(client, db_session)
    ...
    settlement = job_run.result_summary["queue_enqueue"]["enqueue_metadata"][
