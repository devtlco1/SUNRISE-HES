...
def bridge_runtime_operational_closure_to_protocol_execution_intent(
    session: Session,
    *,
    ...
    if attempt.job_run_id is None:
        raise HTTPException(
            ...
            detail="Runtime protocol execution intent requires an attempt linked to a job run.",
        )
    ...
    existing = _load_runtime_protocol_execution_intent(attempt.execution_metadata)
    if existing is not None:
        if existing.session_identifier != payload.session_identifier:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Runtime protocol execution intent is already derived for another session.",
            )
        if existing.executor_identifier != payload.executor_identifier:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Runtime protocol execution intent is already owned by another executor.",
            )
        return RuntimeProtocolExecutionIntentBridgeResponse(
    ...
    if session_result is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution intent requires a finalized runtime session.",
        )
    if session_result.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution intent does not match the finalized runtime session.",
        )
    ...
    if outcome is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution intent requires a recorded runtime execution outcome.",
        )
    if outcome.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution intent does not match the recorded runtime execution outcome.",
        )
    ...
    if disposition is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution intent requires a recorded runtime attempt disposition.",
        )
    if disposition.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution intent does not match the recorded runtime attempt disposition.",
        )
    ...
    if post_processing is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution intent requires a recorded runtime post-processing bridge.",
        )
    if post_processing.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution intent does not match the recorded runtime post-processing bridge.",
        )
    ...
    if materialization is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution intent requires a recorded runtime follow-up materialization.",
        )
    if materialization.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution intent does not match the recorded runtime follow-up materialization.",
        )
    ...
    if closure is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution intent requires a recorded runtime operational closure.",
        )
    ...
    if closure.post_processing_record_id != post_processing.post_processing_record_id:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime operational closure does not match the recorded runtime post-processing bridge.",
        )
    if closure.disposition_record_id != disposition.disposition_record_id:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime operational closure does not match the recorded runtime attempt disposition.",
        )
    if closure.outcome_record_id != outcome.outcome_record_id:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime operational closure does not match the recorded runtime execution outcome.",
        )

    _validate_runtime_protocol_execution_intent_context(
        execution_metadata=attempt.execution_metadata,
        executor_identifier=payload.executor_identifier,
    )

    result = _build_runtime_protocol_execution_intent_result(
        attempt=attempt,
        closure=closure,
        payload=payload,
    )
    intent_payload = {"runtime_protocol_execution_intent": result.model_dump(mode="json")}
    attempt.execution_metadata = _merge_dicts(attempt.execution_metadata, intent_payload)
    command.result_summary = _merge_dicts(command.result_summary, intent_payload)
    job_run.result_summary = _merge_dicts(job_run.result_summary, intent_payload)
...
def _build_runtime_protocol_execution_intent_result(
    *,
    attempt: CommandExecutionAttempt,
    closure: RuntimeOperationalClosureResult,
    payload: RuntimeProtocolExecutionIntentBridgeRequest,
) -> RuntimeProtocolExecutionIntentResult:
    return RuntimeProtocolExecutionIntentResult(
        status=RuntimeProtocolExecutionIntentStatus.DERIVED,
        intent_record_id=(
            "runtime-protocol-execution-intent:"
            f"{attempt.id}:{closure.closure_record_id}"
        ),
        session_identifier=closure.session_identifier,
        closure_record_id=closure.closure_record_id,
        materialization_record_id=closure.materialization_record_id,
        post_processing_record_id=closure.post_processing_record_id,
        disposition_record_id=closure.disposition_record_id,
        outcome_record_id=closure.outcome_record_id,
        executor_identifier=payload.executor_identifier,
        ...
        downstream_state=closure.downstream_state,
        protocol_execution_intent_type=(
            RuntimeProtocolExecutionIntentType.PLACEHOLDER_PROTOCOL_ADAPTER_READY
        ),
        target_execution_mode=RuntimeProtocolExecutionTargetMode.DEFERRED_PROTOCOL_EXECUTION_BOUNDARY,
        included_follow_up_descriptor_types=closure.included_follow_up_descriptor_types,
        intent_recorded_at=datetime.now(UTC).isoformat(),
        intent_recorded_by_executor_identifier=payload.executor_identifier,
        intent_reason=payload.intent_reason,
        already_recorded=False,
        summary=(
            "Runtime operational closure is converted into a durable placeholder "
            "protocol execution intent boundary without invoking any protocol adapter."
        ),
        lineage=closure.lineage,
    )


def _validate_runtime_protocol_execution_intent_context(
    *,
    ...
    if lease is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution intent requires a runtime lease.",
        )
    ...
    if invocation is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution intent requires a runtime invocation gate.",
        )
    ...
    if guard is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution intent requires a runtime execution guard.",
        )
