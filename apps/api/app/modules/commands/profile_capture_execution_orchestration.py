...
from app.modules.commands.schemas import (
    ...
    ProfileCaptureExecutionOrchestrationResult,
...
def orchestrate_profile_capture_command_execution(
    session: Session,
    *,
    ...
    existing_orchestration = _load_profile_capture_execution_orchestration(
    ...
    if command.command_template.category != CommandCategory.PROFILE_CAPTURE:
        raise HTTPException(
            ...
            detail="Selected command is not compatible with the profile-capture execution orchestration slice.",
        )
    if latest_attempt is not None and latest_attempt.ended_at is not None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Profile-capture command already has an incompatible prior finalized execution without a matching orchestration artifact.",
        )
    if command.current_status in {
        ...
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Profile-capture command is not execution-eligible from its current state.",
        )
    _validate_capture_load_profile_normalized_payload(command)

    bootstrap_identifier = _build_profile_capture_orchestration_bootstrap_identifier(
        payload.orchestration_identifier
    )
    bootstrap_response = bootstrap_profile_capture_command_attempt(
        session,
        command_id=command.id,
        payload=ProfileCaptureAttemptBootstrapRequest(
            bootstrap_identifier=bootstrap_identifier,
            bootstrap_reason=payload.orchestration_reason or "profile-capture-execution-orchestration",
        ),
    )
    handoff_response = handoff_profile_capture_command_to_runtime(
        session,
        command_id=command.id,
        payload=ProfileCaptureRuntimeHandoffRequest(
            ...
            handoff_reason=payload.orchestration_reason or "profile-capture-execution-orchestration",
            lease_seconds=payload.lease_seconds,
            session_timeout_seconds=payload.session_timeout_seconds,
        ),
    )
    terminalization_response = terminalize_profile_capture_runtime_execution(
        session,
        command_id=command.id,
        payload=ProfileCaptureRuntimeTerminalizationRequest(
            ...
            or "profile-capture-execution-orchestration",
        ),
    )
    ...
    if attempt is None or refreshed_command is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Profile-capture execution orchestration could not load durable execution state.",
        )
    orchestration_record = _build_profile_capture_execution_orchestration_record(
        command=refreshed_command,
        attempt=attempt,
        payload=payload,
        handoff_record_id=handoff_response.result.runtime_profile_read_execution_record_id,
        terminalization_artifact_present=(
            "profile_capture_runtime_terminalization" in (attempt.execution_metadata or {})
        ),
        orchestrated_at=datetime.now(UTC),
        reused_existing_orchestration=False,
        orchestration_reason_category="executed",
    )
    _persist_profile_capture_execution_orchestration_record(
        session,
        attempt=attempt,
        command=refreshed_command,
        job_run=job_run,
...
def _build_existing_orchestration_response(
    ...
    *,
    ...
    if attempt is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Profile-capture execution orchestration artifact references a missing attempt.",
        )
    if orchestration_record.get("orchestration_identifier") != payload.orchestration_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Profile-capture execution orchestration already exists for another orchestration identifier.",
        )
    if orchestration_record.get("executor_identifier") != payload.executor_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Profile-capture execution orchestration is already owned by another executor.",
        )
...
def _build_orchestration_response(
    *,
    ...
    return ProfileCaptureExecutionOrchestrationResponse(
        result=ProfileCaptureExecutionOrchestrationResult(
            ...
            runtime_profile_read_execution_record_id=str(
                orchestration_record["runtime_profile_read_execution_record_id"]
            ),
...
def _build_profile_capture_execution_orchestration_record(
    *,
    command: MeterCommand,
    attempt: CommandExecutionAttempt,
    payload: ProfileCaptureExecutionOrchestrationRequest,
    ...
    return {
        ...
        "runtime_profile_read_execution_record_id": handoff_record_id,
...
def _persist_profile_capture_execution_orchestration_record(
    session: Session,
    *,
    attempt: CommandExecutionAttempt,
    command: MeterCommand,
    ...
    payload = {"profile_capture_execution_orchestration": orchestration_record}
...
def _build_profile_capture_orchestration_bootstrap_identifier(
    orchestration_identifier: str,
) -> str:
    return f"profile-capture-orchestrator:{orchestration_identifier}"
...
def _load_profile_capture_execution_orchestration(
    execution_metadata: dict[str, object] | None,
) -> dict[str, object] | None:
    if not isinstance(execution_metadata, dict):
        return None
    payload = execution_metadata.get("profile_capture_execution_orchestration")
