...
from app.modules.commands.schemas import (
    ...
    ProfileCaptureRuntimeTerminalizationResult,
)
...
def terminalize_profile_capture_runtime_execution(
    session: Session,
    *,
    ...
    existing_terminalization = _load_profile_capture_runtime_terminalization(
    ...
    if command.command_template.category != CommandCategory.PROFILE_CAPTURE:
        raise HTTPException(
            ...
            detail="Selected command is not compatible with the profile-capture runtime terminalization slice.",
        )
    if attempt is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Profile-capture command does not have a runtime-backed attempt for terminalization.",
        )

    handoff_record = _load_profile_capture_runtime_handoff(attempt.execution_metadata)
    if handoff_record is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Profile-capture attempt is missing the runtime handoff artifact required for terminalization.",
        )
    runtime_execution = _load_runtime_profile_read_execution(attempt.execution_metadata)
    if runtime_execution is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Profile-capture attempt is not yet terminalizable because the runtime profile-read execution artifact is missing.",
        )
    ...
    if terminal_status is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Profile-capture attempt is not yet terminalizable because the runtime capture-load-profile terminal status artifact is missing.",
        )
    _validate_profile_capture_terminalization_context(
        command=command,
        attempt=attempt,
    ...
    mapping = _resolve_terminalization_mapping(
        runtime_execution=runtime_execution,
        terminal_status=terminal_status,
    )
    now = datetime.now(UTC)
    terminalization_record = _build_profile_capture_runtime_terminalization_record(
        command=command,
        attempt=attempt,
        runtime_execution=runtime_execution,
        terminal_status=terminal_status,
    ...
    attempt.execution_metadata = merge_runtime_metadata(
        attempt.execution_metadata,
        {"profile_capture_runtime_terminalization": terminalization_record},
    )
    result_summary = merge_runtime_metadata(
        command.result_summary,
        {"profile_capture_runtime_terminalization": terminalization_record},
    )
    _apply_profile_capture_terminal_state(
        attempt=attempt,
        command=command,
...
def _build_existing_terminalization_response(
    ...
    *,
    ...
    if attempt is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Profile-capture runtime terminalization artifact references a missing attempt.",
        )
    if terminalization_record.get("terminalization_identifier") != payload.terminalization_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Profile-capture runtime terminalization already exists for another terminalization identifier.",
        )
    if terminalization_record.get("executor_identifier") != payload.executor_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Profile-capture runtime terminalization is already owned by another executor.",
        )
    job_run = session.get(JobRun, attempt.job_run_id) if attempt.job_run_id is not None else None
    return _build_terminalization_response(
        command=command,
...
def _validate_profile_capture_terminalization_context(
    *,
    command: MeterCommand,
    attempt: CommandExecutionAttempt,
    handoff_record: dict[str, object],
    runtime_execution: RuntimeProfileReadExecutionResult,
    terminal_status: RuntimeCaptureLoadProfileTerminalStatus,
    executor_identifier: str,
) -> None:
    if attempt.job_run_id is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Profile-capture attempt is not linked to a job run for terminalization.",
        )
    if handoff_record.get("handoff_status") != "handed_off":
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Profile-capture runtime handoff artifact is invalid for terminalization.",
        )
    if handoff_record.get("executor_identifier") != executor_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Profile-capture runtime handoff is owned by another executor.",
        )
    if runtime_execution.command_category != CommandCategory.PROFILE_CAPTURE:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime profile-read execution is incompatible with profile-capture terminalization.",
        )
    if str(runtime_execution.related_command_id) != str(command.id):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime profile-read execution linkage is inconsistent with the related command.",
        )
    if str(runtime_execution.command_attempt_id) != str(attempt.id):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime profile-read execution linkage is inconsistent with the command attempt.",
        )
    if str(runtime_execution.job_run_id) != str(attempt.job_run_id):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime profile-read execution linkage is inconsistent with the job run.",
        )
    if runtime_execution.profile_read_recorded_by_executor_identifier != executor_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime profile-read execution is owned by another executor.",
        )
    if runtime_execution.status.value != "completed":
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Profile-capture runtime execution is not yet terminalizable.",
        )
    if terminal_status.profile_read_execution_record_id != runtime_execution.profile_read_execution_record_id:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal status linkage is inconsistent with the runtime profile-read execution.",
        )
    if terminal_status.command_attempt_id != str(attempt.id):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal status linkage is inconsistent with the command attempt.",
        )
    if terminal_status.session_identifier != runtime_execution.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime terminal status linkage is inconsistent with the runtime session context.",
        )
    handoff_profile_read_execution_record_id = handoff_record.get(
        "runtime_profile_read_execution_record_id"
    )
    if (
        handoff_profile_read_execution_record_id is not None
        and str(handoff_profile_read_execution_record_id)
        != runtime_execution.profile_read_execution_record_id
    ):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime handoff artifact is inconsistent with the recorded runtime profile-read execution.",
        )
...
def _resolve_terminalization_mapping(
    *,
    ...
    if runtime_execution.execution_outcome == RuntimeCommandOutcome.SUCCEEDED:
        if terminal_status.terminal_status != RuntimeCaptureLoadProfileTerminalStatusCategory.ACKNOWLEDGED:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Runtime terminal status is incompatible with a successful profile-capture terminalization.",
            )
        return {
    ...
    if runtime_execution.execution_outcome == RuntimeCommandOutcome.TIMED_OUT:
        return {
            ...
            "error_code": "PROFILE_CAPTURE_RUNTIME_TIMED_OUT",
            "error_message": runtime_execution.error_detail
            or terminal_status.summary
            or runtime_execution.summary,
        }
    return {
        ...
        "error_code": f"PROFILE_CAPTURE_{terminal_status.terminal_status.value.upper()}",
        "error_message": runtime_execution.error_detail
        or terminal_status.summary
        or runtime_execution.summary,
    }
...
def _build_profile_capture_runtime_terminalization_record(
    *,
    command: MeterCommand,
    attempt: CommandExecutionAttempt,
    runtime_execution: RuntimeProfileReadExecutionResult,
    ...
    return {
        ...
        "executor_identifier": payload.executor_identifier,
        "runtime_profile_read_execution_record_id": runtime_execution.profile_read_execution_record_id,
        "runtime_capture_load_profile_terminal_status": terminal_status.terminal_status.value,
...
def _apply_profile_capture_terminal_state(
    *,
    attempt: CommandExecutionAttempt,
    command: MeterCommand,
    job_run: JobRun | None,
...
def _build_terminalization_response(
    *,
    ...
    return ProfileCaptureRuntimeTerminalizationResponse(
        result=ProfileCaptureRuntimeTerminalizationResult(
            ...
            runtime_profile_read_execution_record_id=str(
                terminalization_record["runtime_profile_read_execution_record_id"]
            ),
            runtime_capture_load_profile_terminal_status=str(
                terminalization_record["runtime_capture_load_profile_terminal_status"]
            ),
            attempt_final_status=attempt.status,
            command_final_status=command.current_status,
            job_run_final_status=job_run.status if job_run is not None else None,
...
def _load_profile_capture_runtime_terminalization(
    execution_metadata: dict[str, object] | None,
) -> dict[str, object] | None:
    if not isinstance(execution_metadata, dict):
        return None
    payload = execution_metadata.get("profile_capture_runtime_terminalization")
    return payload if isinstance(payload, dict) else None
