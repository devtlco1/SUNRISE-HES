...
def bridge_runtime_publication_contract_to_externalization_envelope(
    session: Session,
    *,
    ...
    if attempt.job_run_id is None:
        raise HTTPException(
            ...
            detail="Runtime externalization envelope requires an attempt linked to a job run.",
        )
    ...
    existing = _load_runtime_externalization_envelope(attempt.execution_metadata)
    if existing is not None:
        if existing.session_identifier != payload.session_identifier:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Runtime externalization envelope is already recorded for another session.",
            )
        if existing.executor_identifier != payload.executor_identifier:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Runtime externalization envelope is already owned by another executor.",
            )
        return RuntimeExternalizationEnvelopeBridgeResponse(
    ...
    if session_result is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope requires a finalized runtime session.",
        )
    if session_result.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope does not match the finalized runtime session.",
        )
    ...
    if outcome is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope requires a recorded runtime execution outcome.",
        )
    if outcome.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope does not match the recorded runtime execution outcome.",
        )
    ...
    if disposition is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope requires a recorded runtime attempt disposition.",
        )
    if disposition.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope does not match the recorded runtime attempt disposition.",
        )
    ...
    if post_processing is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope requires a recorded runtime post-processing bridge.",
        )
    if post_processing.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope does not match the recorded runtime post-processing bridge.",
        )
    ...
    if materialization is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope requires a recorded runtime follow-up materialization.",
        )
    if materialization.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope does not match the recorded runtime follow-up materialization.",
        )
    ...
    if closure is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope requires a recorded runtime operational closure.",
        )
    if closure.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope does not match the recorded runtime operational closure.",
        )
    ...
    if intent is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope requires a recorded runtime protocol execution intent.",
        )
    if intent.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope does not match the recorded runtime protocol execution intent.",
        )
    ...
    if selection is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope requires a recorded runtime protocol adapter selection.",
        )
    if selection.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope does not match the recorded runtime protocol adapter selection.",
        )
    ...
    if dispatch_request is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope requires a recorded runtime protocol dispatch request.",
        )
    if dispatch_request.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope does not match the recorded runtime protocol dispatch request.",
        )
    ...
    if invocation_result is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope requires a recorded runtime protocol invocation result.",
        )
    if invocation_result.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope does not match the recorded runtime protocol invocation result.",
        )
    ...
    if observation is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope requires a recorded runtime protocol execution observation.",
        )
    if observation.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope does not match the recorded runtime protocol execution observation.",
        )
    ...
    if interpretation is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope requires a recorded runtime protocol interpretation.",
        )
    if interpretation.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope does not match the recorded runtime protocol interpretation.",
        )
    ...
    if reconciliation is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope requires a recorded runtime protocol reconciliation.",
        )
    if reconciliation.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope does not match the recorded runtime protocol reconciliation.",
        )
    ...
    if settlement is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope requires a recorded runtime terminal settlement.",
        )
    if settlement.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope does not match the recorded runtime terminal settlement.",
        )
    ...
    if attestation is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope requires a recorded runtime closure attestation.",
        )
    if attestation.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope does not match the recorded runtime closure attestation.",
        )
    ...
    if publication_contract is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope requires a recorded runtime publication contract.",
        )
    if publication_contract.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope does not match the recorded runtime publication contract.",
        )
    ...
    if publication_contract.attestation_record_id != attestation.attestation_record_id:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime publication contract does not match the recorded runtime closure attestation.",
        )

    _validate_runtime_externalization_envelope_context(
        execution_metadata=attempt.execution_metadata,
        executor_identifier=payload.executor_identifier,
    )

    result = _build_runtime_externalization_envelope_result(
        attempt=attempt,
        publication_contract=publication_contract,
        payload=payload,
    )
    envelope_payload = {"runtime_externalization_envelope": result.model_dump(mode="json")}
    ...
    job_run.result_summary = merge_runtime_metadata(job_run.result_summary, envelope_payload)
...
def _build_runtime_externalization_envelope_result(
    *,
    attempt: CommandExecutionAttempt,
    publication_contract: RuntimePublicationContractResult,
    payload: RuntimeExternalizationEnvelopeBridgeRequest,
) -> RuntimeExternalizationEnvelopeResult:
    publication_payload = publication_contract.publication_contract_payload
    return RuntimeExternalizationEnvelopeResult(
        status=RuntimeExternalizationEnvelopeStatus.RECORDED,
        envelope_record_id=(
            "runtime-externalization-envelope:"
            f"{attempt.id}:{publication_contract.publication_contract_record_id}"
        ),
        session_identifier=publication_contract.session_identifier,
        publication_contract_record_id=publication_contract.publication_contract_record_id,
        attestation_record_id=publication_contract.attestation_record_id,
        settlement_record_id=publication_contract.settlement_record_id,
        reconciliation_record_id=publication_contract.reconciliation_record_id,
        interpretation_record_id=publication_contract.interpretation_record_id,
        observation_record_id=publication_contract.observation_record_id,
        invocation_result_record_id=publication_contract.invocation_result_record_id,
        dispatch_request_record_id=publication_contract.dispatch_request_record_id,
        selection_record_id=publication_contract.selection_record_id,
        intent_record_id=publication_contract.intent_record_id,
        closure_record_id=publication_contract.closure_record_id,
        materialization_record_id=publication_contract.materialization_record_id,
        post_processing_record_id=publication_contract.post_processing_record_id,
        disposition_record_id=publication_contract.disposition_record_id,
        outcome_record_id=publication_contract.outcome_record_id,
        executor_identifier=payload.executor_identifier,
        job_run_id=str(attempt.job_run_id),
        related_command_id=str(attempt.meter_command_id),
        command_attempt_id=str(attempt.id),
        terminal_outcome=publication_contract.terminal_outcome,
        downstream_state=publication_contract.downstream_state,
        envelope_key="placeholder.runtime.externalization-envelope",
        envelope_family=(
            RuntimeExternalizationEnvelopeFamily.PLACEHOLDER_RUNTIME_EXTERNALIZATION_ENVELOPE
        ),
        envelope_payload=RuntimeExternalizationEnvelopePayload(
            schema_version="placeholder-runtime-externalization-envelope.v1",
            envelope_state=(
                RuntimeExternalizationEnvelopeState.PLACEHOLDER_EXTERNALIZATION_ENVELOPE_RECORDED
            ),
            delivery_projection_mode=publication_payload.publication_projection_mode,
            adapter_family=publication_payload.adapter_family,
            capability_profile=publication_payload.capability_profile,
            delivery_readiness_classification=(
                RuntimeExternalizationEnvelopeClassification.PLACEHOLDER_RUNTIME_DELIVERY_READINESS_READY
            ),
            target_channel_family=(
                RuntimeExternalizationTargetChannelFamily.PLACEHOLDER_EXTERNAL_RUNTIME_DELIVERY
            ),
            placeholder_chain_references={
                "publication_contract_record_id": (
                    publication_contract.publication_contract_record_id
                ),
                "attestation_record_id": publication_contract.attestation_record_id,
                "settlement_record_id": publication_contract.settlement_record_id,
                "reconciliation_record_id": publication_contract.reconciliation_record_id,
        ...
        included_follow_up_descriptor_types=publication_contract.included_follow_up_descriptor_types,
        envelope_recorded_at=datetime.now(UTC).isoformat(),
        envelope_recorded_by_executor_identifier=payload.executor_identifier,
        envelope_reason=payload.envelope_reason,
        already_recorded=False,
        summary=(
            "Runtime publication contract is converted into a durable placeholder "
            "delivery-ready externalization envelope without executing any live publication or delivery logic."
        ),
        lineage=publication_contract.lineage,
    )


def _validate_runtime_externalization_envelope_context(
    *,
    ...
    if lease is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope requires a runtime lease.",
        )
    ...
    if invocation is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope requires a runtime invocation gate.",
        )
    ...
    if guard is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime externalization envelope requires a runtime execution guard.",
        )
