...
def bridge_runtime_externalization_envelope_to_delivery_contract(
    session: Session,
    *,
    ...
    if attempt.job_run_id is None:
        raise HTTPException(
            ...
            detail="Runtime delivery contract requires an attempt linked to a job run.",
        )
    ...
    existing = _load_runtime_delivery_contract(attempt.execution_metadata)
    if existing is not None:
        if existing.session_identifier != payload.session_identifier:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Runtime delivery contract is already recorded for another session.",
            )
        if existing.executor_identifier != payload.executor_identifier:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Runtime delivery contract is already owned by another executor.",
            )
        return RuntimeDeliveryContractBridgeResponse(
    ...
    if session_result is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract requires a finalized runtime session.",
        )
    if session_result.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract does not match the finalized runtime session.",
        )
    ...
    if outcome is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract requires a recorded runtime execution outcome.",
        )
    if outcome.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract does not match the recorded runtime execution outcome.",
        )
    ...
    if disposition is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract requires a recorded runtime attempt disposition.",
        )
    if disposition.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract does not match the recorded runtime attempt disposition.",
        )
    ...
    if post_processing is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract requires a recorded runtime post-processing bridge.",
        )
    if post_processing.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract does not match the recorded runtime post-processing bridge.",
        )
    ...
    if materialization is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract requires a recorded runtime follow-up materialization.",
        )
    if materialization.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract does not match the recorded runtime follow-up materialization.",
        )
    ...
    if closure is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract requires a recorded runtime operational closure.",
        )
    if closure.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract does not match the recorded runtime operational closure.",
        )
    ...
    if intent is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract requires a recorded runtime protocol execution intent.",
        )
    if intent.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract does not match the recorded runtime protocol execution intent.",
        )
    ...
    if selection is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract requires a recorded runtime protocol adapter selection.",
        )
    if selection.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract does not match the recorded runtime protocol adapter selection.",
        )
    ...
    if dispatch_request is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract requires a recorded runtime protocol dispatch request.",
        )
    if dispatch_request.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract does not match the recorded runtime protocol dispatch request.",
        )
    ...
    if invocation_result is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract requires a recorded runtime protocol invocation result.",
        )
    if invocation_result.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract does not match the recorded runtime protocol invocation result.",
        )
    ...
    if observation is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract requires a recorded runtime protocol execution observation.",
        )
    if observation.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract does not match the recorded runtime protocol execution observation.",
        )
    ...
    if interpretation is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract requires a recorded runtime protocol interpretation.",
        )
    if interpretation.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract does not match the recorded runtime protocol interpretation.",
        )
    ...
    if reconciliation is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract requires a recorded runtime protocol reconciliation.",
        )
    if reconciliation.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract does not match the recorded runtime protocol reconciliation.",
        )
    ...
    if settlement is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract requires a recorded runtime terminal settlement.",
        )
    if settlement.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract does not match the recorded runtime terminal settlement.",
        )
    ...
    if attestation is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract requires a recorded runtime closure attestation.",
        )
    if attestation.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract does not match the recorded runtime closure attestation.",
        )
    ...
    if publication_contract is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract requires a recorded runtime publication contract.",
        )
    if publication_contract.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract does not match the recorded runtime publication contract.",
        )
    ...
    envelope = _load_runtime_externalization_envelope(attempt.execution_metadata)
    if envelope is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract requires a recorded runtime externalization envelope.",
        )
    ...
    if (
        envelope.publication_contract_record_id
    ...
    _validate_runtime_delivery_contract_context(
        execution_metadata=attempt.execution_metadata,
        executor_identifier=payload.executor_identifier,
    )

    result = _build_runtime_delivery_contract_result(
        attempt=attempt,
        envelope=envelope,
        payload=payload,
    )
    delivery_payload = {"runtime_delivery_contract": result.model_dump(mode="json")}
    ...
    job_run.result_summary = merge_runtime_metadata(job_run.result_summary, delivery_payload)
...
def _build_runtime_delivery_contract_result(
    *,
    attempt: CommandExecutionAttempt,
    envelope: RuntimeExternalizationEnvelopeResult,
    payload: RuntimeDeliveryContractBridgeRequest,
) -> RuntimeDeliveryContractResult:
    envelope_payload = envelope.envelope_payload
    return RuntimeDeliveryContractResult(
        status=RuntimeDeliveryContractStatus.RECORDED,
        delivery_contract_record_id=(
            "runtime-delivery-contract:"
            f"{attempt.id}:{envelope.envelope_record_id}"
        ),
        session_identifier=envelope.session_identifier,
        envelope_record_id=envelope.envelope_record_id,
        publication_contract_record_id=envelope.publication_contract_record_id,
        attestation_record_id=envelope.attestation_record_id,
        settlement_record_id=envelope.settlement_record_id,
        reconciliation_record_id=envelope.reconciliation_record_id,
        interpretation_record_id=envelope.interpretation_record_id,
        observation_record_id=envelope.observation_record_id,
        invocation_result_record_id=envelope.invocation_result_record_id,
        dispatch_request_record_id=envelope.dispatch_request_record_id,
        selection_record_id=envelope.selection_record_id,
        intent_record_id=envelope.intent_record_id,
        closure_record_id=envelope.closure_record_id,
        materialization_record_id=envelope.materialization_record_id,
        post_processing_record_id=envelope.post_processing_record_id,
        disposition_record_id=envelope.disposition_record_id,
        ...
        downstream_state=envelope.downstream_state,
        delivery_contract_key="placeholder.runtime.delivery-contract",
        delivery_contract_family=(
            RuntimeDeliveryContractFamily.PLACEHOLDER_RUNTIME_DELIVERY_CONTRACT
        ),
        delivery_contract_payload=RuntimeDeliveryContractPayload(
            schema_version="placeholder-runtime-delivery-contract.v1",
            delivery_state=(
                RuntimeDeliveryContractState.PLACEHOLDER_DISPATCH_READINESS_RECORDED
            ),
            dispatch_readiness_mode=envelope_payload.delivery_projection_mode,
            adapter_family=envelope_payload.adapter_family,
            capability_profile=envelope_payload.capability_profile,
            dispatch_readiness_classification=(
                RuntimeDeliveryContractClassification.PLACEHOLDER_RUNTIME_DELIVERY_CONTRACT_READY
            ),
            delivery_target_family=(
                RuntimeDeliveryTargetFamily.PLACEHOLDER_EXTERNAL_DELIVERY_DISPATCH
            ),
        ...
        included_follow_up_descriptor_types=envelope.included_follow_up_descriptor_types,
        delivery_contract_recorded_at=datetime.now(UTC).isoformat(),
        delivery_contract_recorded_by_executor_identifier=payload.executor_identifier,
        delivery_contract_reason=payload.delivery_contract_reason,
        already_recorded=False,
        summary=(
            "Runtime externalization envelope is converted into a durable placeholder "
            "dispatch-ready delivery contract without executing any live publication or delivery logic."
        ),
        lineage=envelope.lineage,
    )


def _validate_runtime_delivery_contract_context(
    *,
    ...
    if lease is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract requires a runtime lease.",
        )
    ...
    if invocation is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract requires a runtime invocation gate.",
        )
    ...
    if guard is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime delivery contract requires a runtime execution guard.",
        )
