...
def bridge_runtime_protocol_dispatch_request_to_invocation_result(
    session: Session,
    *,
    ...
    if attempt.job_run_id is None:
        raise HTTPException(
            ...
            detail="Runtime protocol invocation result requires an attempt linked to a job run.",
        )
    ...
    existing = _load_runtime_protocol_invocation_result(attempt.execution_metadata)
    if existing is not None:
        if existing.session_identifier != payload.session_identifier:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Runtime protocol invocation result is already recorded for another session.",
            )
        if existing.executor_identifier != payload.executor_identifier:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Runtime protocol invocation result is already owned by another executor.",
            )
        return RuntimeProtocolInvocationResultBridgeResponse(
    ...
    if session_result is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol invocation result requires a finalized runtime session.",
        )
    if session_result.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol invocation result does not match the finalized runtime session.",
        )
    ...
    if outcome is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol invocation result requires a recorded runtime execution outcome.",
        )
    if outcome.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol invocation result does not match the recorded runtime execution outcome.",
        )
    ...
    if disposition is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol invocation result requires a recorded runtime attempt disposition.",
        )
    if disposition.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol invocation result does not match the recorded runtime attempt disposition.",
        )
    ...
    if post_processing is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol invocation result requires a recorded runtime post-processing bridge.",
        )
    if post_processing.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol invocation result does not match the recorded runtime post-processing bridge.",
        )
    ...
    if materialization is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol invocation result requires a recorded runtime follow-up materialization.",
        )
    if materialization.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol invocation result does not match the recorded runtime follow-up materialization.",
        )
    ...
    if closure is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol invocation result requires a recorded runtime operational closure.",
        )
    if closure.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol invocation result does not match the recorded runtime operational closure.",
        )
    ...
    if intent is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol invocation result requires a recorded runtime protocol execution intent.",
        )
    if intent.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol invocation result does not match the recorded runtime protocol execution intent.",
        )
    ...
    if selection is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol invocation result requires a recorded runtime protocol adapter selection.",
        )
    if selection.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol invocation result does not match the recorded runtime protocol adapter selection.",
        )
    ...
    if dispatch_request is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol invocation result requires a recorded runtime protocol dispatch request.",
        )
    ...
    if dispatch_request.selection_record_id != selection.selection_record_id:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol dispatch request does not match the recorded runtime protocol adapter selection.",
        )

    _validate_runtime_protocol_invocation_result_context(
        execution_metadata=attempt.execution_metadata,
        executor_identifier=payload.executor_identifier,
    )

    result = _build_runtime_protocol_invocation_result(
        attempt=attempt,
        dispatch_request=dispatch_request,
        payload=payload,
    )
    invocation_payload = {"runtime_protocol_invocation_result": result.model_dump(mode="json")}
    attempt.execution_metadata = _merge_dicts(attempt.execution_metadata, invocation_payload)
    command.result_summary = _merge_dicts(command.result_summary, invocation_payload)
    job_run.result_summary = _merge_dicts(job_run.result_summary, invocation_payload)
...
def _build_runtime_protocol_invocation_result(
    *,
    attempt: CommandExecutionAttempt,
    dispatch_request: RuntimeProtocolDispatchRequestResult,
    payload: RuntimeProtocolInvocationResultBridgeRequest,
) -> RuntimeProtocolInvocationResult:
    envelope = dispatch_request.execution_envelope
    return RuntimeProtocolInvocationResult(
        status=RuntimeProtocolInvocationResultStatus.RECORDED,
        invocation_result_record_id=(
            "runtime-protocol-invocation-result:"
            f"{attempt.id}:{dispatch_request.dispatch_request_record_id}"
        ),
        session_identifier=dispatch_request.session_identifier,
        dispatch_request_record_id=dispatch_request.dispatch_request_record_id,
        selection_record_id=dispatch_request.selection_record_id,
        intent_record_id=dispatch_request.intent_record_id,
        closure_record_id=dispatch_request.closure_record_id,
        materialization_record_id=dispatch_request.materialization_record_id,
        post_processing_record_id=dispatch_request.post_processing_record_id,
        disposition_record_id=dispatch_request.disposition_record_id,
        outcome_record_id=dispatch_request.outcome_record_id,
        executor_identifier=payload.executor_identifier,
        job_run_id=str(attempt.job_run_id),
        related_command_id=str(attempt.meter_command_id),
        command_attempt_id=str(attempt.id),
        terminal_outcome=dispatch_request.terminal_outcome,
        downstream_state=dispatch_request.downstream_state,
        invocation_result_key="placeholder.protocol.adapter.invocation-result",
        result_family=RuntimeProtocolInvocationResultFamily.PLACEHOLDER_PROTOCOL_INVOCATION_RESULT,
        invocation_payload=RuntimeProtocolInvocationPayload(
            schema_version="placeholder-runtime-protocol-invocation-result.v1",
            acknowledgment_state=(
                RuntimeProtocolInvocationAcknowledgmentState.PLACEHOLDER_PROTOCOL_INVOCATION_ACKNOWLEDGED
            ),
            invocation_mode=envelope.target_mode,
        ...
        included_follow_up_descriptor_types=dispatch_request.included_follow_up_descriptor_types,
        result_recorded_at=datetime.now(UTC).isoformat(),
        result_recorded_by_executor_identifier=payload.executor_identifier,
        result_reason=payload.result_reason,
        already_recorded=False,
        summary=(
            "Runtime protocol dispatch request is converted into a durable placeholder "
            "adapter invocation acknowledgment without executing any live protocol adapter."
        ),
        lineage=dispatch_request.lineage,
    )


def _validate_runtime_protocol_invocation_result_context(
    *,
    ...
    if lease is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol invocation result requires a runtime lease.",
        )
    ...
    if invocation is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol invocation result requires a runtime invocation gate.",
        )
    ...
    if guard is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol invocation result requires a runtime execution guard.",
        )
