...
def test_redis_worker_retention_register_maps_into_stable_worker_register_reconciliation_records_correctly(
    client,
    db_session: Session,
) -> None:
    ...
    retention_register = map_worker_closure_archive_to_retention_register(
        map_worker_ledger_closure_to_closure_archive(
            map_worker_final_attestation_ledger_to_ledger_closure(
                map_worker_attestation_seal_to_final_attestation_ledger(
                    map_worker_completion_attestation_to_attestation_seal(
    ...
    reconciliation = map_worker_retention_register_to_register_reconciliation(
        retention_register
    )

    assert reconciliation.records[0].reconciliation_type == "noop_register_reconciliation"
    assert reconciliation.records[1].reconciliation_type == "register_reconciliation_summary"
    assert reconciliation.records[2].reconciliation_type == "reconciliation_action_record"
    assert reconciliation.records[3].reconciliation_type == "register_close_note"
    assert reconciliation.records[4].reconciliation_type == "final_register_close_artifact"
    assert reconciliation.total_records == 5
    assert reconciliation.terminal_records == 4
    assert reconciliation.reconciliation_ready is True


def test_register_reconciliation_action_note_and_final_close_artifacts_are_exposed_correctly(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
    ...
    reconciliation_values = [record.reconciliation_type for record in reconciliation.records]
    assert reconciliation_values == [
        "noop_register_reconciliation",
        "register_reconciliation_summary",
        "reconciliation_action_record",
        "register_close_note",
        "final_register_close_artifact",
    ]


def test_lineage_and_correlation_are_preserved_through_worker_register_reconciliation_normalization(
    client,
    db_session: Session,
) -> None:
    token = _login_as_super_admin(client, db_session)
    ...
    assert reconciliation.metadata["source_identifiers"]["job_run_id"] == job_run_id
    assert reconciliation.metadata["source_identifiers"]["command_id"] is not None
    assert (
        reconciliation.metadata["correlation_lineage"]["derived_correlation_id"]
...
def test_repeated_enqueue_calls_stay_idempotent_with_worker_register_reconciliation_generation(
    client,
    db_session: Session,
    monkeypatch,
) -> None:
...
def test_summaries_remain_consistent_after_worker_register_reconciliation_generation(
    client,
    db_session: Session,
    monkeypatch,
) -> None:
    ...
    reconciliation = job_run.result_summary["queue_enqueue"]["enqueue_metadata"][
        "redis_worker_register_reconciliation"
    ]
    assert (
        reconciliation["records"][0]["reconciliation_type"]
        == "noop_register_reconciliation"
    )
    assert (
        reconciliation["records"][1]["reconciliation_type"]
        == "register_reconciliation_summary"
    )
    assert reconciliation["records"][3]["reconciliation_type"] == "register_close_note"
    assert reconciliation["total_records"] == 5
