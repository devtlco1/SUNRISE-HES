...
def bridge_runtime_protocol_invocation_result_to_execution_observation(
    session: Session,
    *,
    ...
    if attempt.job_run_id is None:
        raise HTTPException(
            ...
            detail="Runtime protocol execution observation requires an attempt linked to a job run.",
        )
    ...
    existing = _load_runtime_protocol_execution_observation(attempt.execution_metadata)
    if existing is not None:
        if existing.session_identifier != payload.session_identifier:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Runtime protocol execution observation is already recorded for another session.",
            )
        if existing.executor_identifier != payload.executor_identifier:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Runtime protocol execution observation is already owned by another executor.",
            )
        return RuntimeProtocolExecutionObservationBridgeResponse(
    ...
    if session_result is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution observation requires a finalized runtime session.",
        )
    if session_result.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution observation does not match the finalized runtime session.",
        )
    ...
    if outcome is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution observation requires a recorded runtime execution outcome.",
        )
    if outcome.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution observation does not match the recorded runtime execution outcome.",
        )
    ...
    if disposition is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution observation requires a recorded runtime attempt disposition.",
        )
    if disposition.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution observation does not match the recorded runtime attempt disposition.",
        )
    ...
    if post_processing is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution observation requires a recorded runtime post-processing bridge.",
        )
    if post_processing.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution observation does not match the recorded runtime post-processing bridge.",
        )
    ...
    if materialization is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution observation requires a recorded runtime follow-up materialization.",
        )
    if materialization.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution observation does not match the recorded runtime follow-up materialization.",
        )
    ...
    if closure is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution observation requires a recorded runtime operational closure.",
        )
    if closure.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution observation does not match the recorded runtime operational closure.",
        )
    ...
    if intent is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution observation requires a recorded runtime protocol execution intent.",
        )
    if intent.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution observation does not match the recorded runtime protocol execution intent.",
        )
    ...
    if selection is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution observation requires a recorded runtime protocol adapter selection.",
        )
    if selection.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution observation does not match the recorded runtime protocol adapter selection.",
        )
    ...
    if dispatch_request is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution observation requires a recorded runtime protocol dispatch request.",
        )
    if dispatch_request.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution observation does not match the recorded runtime protocol dispatch request.",
        )
    ...
    if invocation_result is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution observation requires a recorded runtime protocol invocation result.",
        )
    if invocation_result.session_identifier != payload.session_identifier:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution observation does not match the recorded runtime protocol invocation result.",
        )
    ...
    if invocation_result.dispatch_request_record_id != dispatch_request.dispatch_request_record_id:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol invocation result does not match the recorded runtime protocol dispatch request.",
        )

    _validate_runtime_protocol_execution_observation_context(
        execution_metadata=attempt.execution_metadata,
        executor_identifier=payload.executor_identifier,
    )

    result = _build_runtime_protocol_execution_observation_result(
        attempt=attempt,
        invocation_result=invocation_result,
        payload=payload,
    )
    observation_payload = {"runtime_protocol_execution_observation": result.model_dump(mode="json")}
    attempt.execution_metadata = _merge_dicts(attempt.execution_metadata, observation_payload)
    command.result_summary = _merge_dicts(command.result_summary, observation_payload)
    job_run.result_summary = _merge_dicts(job_run.result_summary, observation_payload)
...
def _build_runtime_protocol_execution_observation_result(
    *,
    attempt: CommandExecutionAttempt,
    invocation_result: RuntimeProtocolInvocationResult,
    payload: RuntimeProtocolExecutionObservationBridgeRequest,
) -> RuntimeProtocolExecutionObservationResult:
    observation = invocation_result.invocation_payload
    return RuntimeProtocolExecutionObservationResult(
        status=RuntimeProtocolExecutionObservationStatus.RECORDED,
        observation_record_id=(
            "runtime-protocol-execution-observation:"
            f"{attempt.id}:{invocation_result.invocation_result_record_id}"
        ),
        session_identifier=invocation_result.session_identifier,
        invocation_result_record_id=invocation_result.invocation_result_record_id,
        dispatch_request_record_id=invocation_result.dispatch_request_record_id,
        selection_record_id=invocation_result.selection_record_id,
        intent_record_id=invocation_result.intent_record_id,
        closure_record_id=invocation_result.closure_record_id,
        materialization_record_id=invocation_result.materialization_record_id,
        post_processing_record_id=invocation_result.post_processing_record_id,
        disposition_record_id=invocation_result.disposition_record_id,
        outcome_record_id=invocation_result.outcome_record_id,
        executor_identifier=payload.executor_identifier,
        job_run_id=str(attempt.job_run_id),
        related_command_id=str(attempt.meter_command_id),
        command_attempt_id=str(attempt.id),
        terminal_outcome=invocation_result.terminal_outcome,
        downstream_state=invocation_result.downstream_state,
        observation_key="placeholder.protocol.execution-observation",
        observation_family=(
            RuntimeProtocolExecutionObservationFamily.PLACEHOLDER_PROTOCOL_EXECUTION_OBSERVATION
        ),
        observation_payload=RuntimeProtocolExecutionObservationPayload(
            schema_version="placeholder-runtime-protocol-execution-observation.v1",
            normalization_state=(
                RuntimeProtocolNormalizationState.PLACEHOLDER_PROTOCOL_RESPONSE_NORMALIZED
            ),
            observation_mode=observation.invocation_mode,
            adapter_family=observation.adapter_family,
            capability_profile=observation.capability_profile,
        ...
        included_follow_up_descriptor_types=invocation_result.included_follow_up_descriptor_types,
        observation_recorded_at=datetime.now(UTC).isoformat(),
        observation_recorded_by_executor_identifier=payload.executor_identifier,
        observation_reason=payload.observation_reason,
        already_recorded=False,
        summary=(
            "Runtime protocol invocation result is converted into a durable placeholder "
            "normalized execution observation without executing any live protocol response handling."
        ),
        lineage=invocation_result.lineage,
    )


def _validate_runtime_protocol_execution_observation_context(
    *,
    ...
    if lease is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution observation requires a runtime lease.",
        )
    ...
    if invocation is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution observation requires a runtime invocation gate.",
        )
    ...
    if guard is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Runtime protocol execution observation requires a runtime execution guard.",
        )
